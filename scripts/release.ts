...
export function parseReleaseRequest(
  ...
  if (command !== "check" && command !== "prepare") {
    throw new Error(
      "Usage: release.ts <check|prepare> <package-name|directory> <v<semver>> [destination].",
    );
  }
  const entry = selector === undefined ? undefined : packageFor(selector);
  if (!entry) {
    throw new Error(
      `Select exactly one public package: ${PUBLIC_PACKAGES.map(({ name }) => name).join(", ")}.`,
    );
  }
  const tag = argumentTag ?? environment.RELEASE_TAG;
  if (!tag)
    throw new Error("A release tag is required, for example v0.1.0-beta.1.");
  versionFromTag(tag);
  return {
    command,
    entry,
...
function runHostGate(entry: PublicPackage): void {
  execFileSync(
    "corepack",
    ["pnpm", "exec", "tsx", "scripts/artifact-smoke.ts", entry.name],
    {
      cwd: root,
      env: process.env,
      stdio: "inherit",
...
async function main(): Promise<void> {
  ...
  if (request.command === "prepare") {
    ...
    console.log(
      JSON.stringify(
        {
          package: request.entry.name,
          tag: request.tag,
          channel: npmDistTag(request.tag),
          artifact,
        },
        null,
        2,
    ...
    console.log(
      `${request.entry.name} release check passed for ${request.tag} (${npmDistTag(request.tag)}).`,
    );
  }
}
