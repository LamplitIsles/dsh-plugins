...
export function checkRegistryVersion(
  ...
  if (Object.hasOwn(registry.versions ?? {}, version)) {
    throw new Error(
      `Version ${version} is already published; choose a new version.`,
    );
  }
  ...
  if (npmDistTag(`v${version}`) === "latest" && latest) {
    ...
    if (
      sameStable ||
      (differing >= 0 && candidate[differing]! < current[differing]!)
    ) {
      throw new Error(
        `Stable version ${version} must be newer than npm latest ${latest}.`,
      );
    }
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `${request.entry.name}@${version}: unpublished, channel ${channel}`,
  );
}
