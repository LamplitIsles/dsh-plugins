...
function checkExpandedManifest(
  ...
  if (manifest.name !== entry.name)
    errors.push(`${entry.name} packed package name is incorrect.`);
  if (manifest.private === true)
    errors.push(`${entry.name} packed artifact is private.`);
  const serialized = JSON.stringify(manifest);
  if (serialized.includes("workspace:") || serialized.includes("catalog:")) {
    errors.push(
      `${entry.name} packed manifest contains an unresolved workspace protocol.`,
    );
  }
    ...
    for (const [name, version] of Object.entries(manifest[section] ?? {})) {
      if (name.startsWith("@deepseek-ai/dsh-") && version !== DSH_RC_VERSION) {
        const displayedVersion =
          typeof version === "string" ? version : "<invalid>";
        errors.push(
          `${entry.name} has a non-rc.1 ${section} ${name}@${displayedVersion}.`,
        );
      }
    }
  }
  for (const dependency of [
    ...
    if (manifest.peerDependencies?.[dependency] !== expected) {
      errors.push(
        `${entry.name} lacks ${dependency}@${expected} in peerDependencies.`,
      );
    }
  }
  for (const section of ["dependencies", "optionalDependencies"]) {
    for (const name of Object.keys(manifest[section] ?? {})) {
      if (name.startsWith("@deepseek-ai/")) {
        errors.push(
          `${entry.name} ships a Host dependency in ${section}: ${name}.`,
        );
      }
    }
  }
...
const temporaryDirectory = await mkdtemp(
  join(tmpdir(), "dsh-plugins-pack-check-"),
);
try {
  ...
  for (const entry of PUBLIC_PACKAGES) {
    ...
    requireCondition(
      ...
      `${entry.name} artifact version differs from its source manifest.`,
    );
