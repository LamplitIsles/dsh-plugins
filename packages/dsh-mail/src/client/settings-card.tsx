...
import type { MailSettings, ConnectionState } from "../constants.js";
import {
...
function snapshotValue(
  scope: SettingsScope<Partial<MailSettings>>,
): SettingsScopeSnapshot<Partial<MailSettings>> {
...
export function MailSettingsCard(
  ...
  useEffect(() => {
    ...
    const refresh = async (): Promise<void> => {
      try {
        const result = await props.connection.rpc.call(
          RPC_CHANNEL,
          RPC_STATUS,
          {},
        );
  ...
  useEffect(() => {
    ...
    const timer = globalThis.setInterval(() => {
      void props.connection.rpc
        .call(RPC_CHANNEL, RPC_STATUS, {})
        .then((result) => {
          if (!disposed) setStatus(safeStatus(unwrapRpc(result)));
        })
        .catch((error: unknown) => {
  ...
  const start = async (force = false): Promise<void> => {
    ...
    const authorizationWindow = props.openUrl
      ? null
    ...
    try {
      ...
      setStatus({
        ...
        retryable: true,
        message:
          error instanceof Error ? error.message : "Mail connection failed.",
      });
    } finally {
      setBusy(false);
    }
  ...
  const saveSettings = async (): Promise<void> => {
    ...
    try {
      await props.scope.mutate([
        ...
        {
          op: "set",
          path: ["upstreamEndpoint"],
          value: values.upstreamEndpoint,
        },
        {
          op: "set",
          path: ["oauthAuthorizationServer"],
          value: values.oauthAuthorizationServer,
        },
        {
          op: "set",
          path: ["oauthCallbackUrl"],
      ...
      setSaveError(
        error instanceof Error
          ? error.message
          : "Could not save mailbox address.",
      );
    } finally {
      setBusy(false);
    }
    ...
    >
      ...
      >
        <span className={css.headText}>
          <span id={titleId} className={css.name}>
            Agent mailbox
          </span>
          <span className={css.description}>
            Send and manage mail through the configured mailbox.
          </span>
        </span>
      ...
      {open ? (
        <div id={descriptionId} className={css.body} aria-labelledby={titleId}>
          <div className={css.field}>
            <label className={css.mailboxLabel} htmlFor={`${titleId}-mailbox`}>
              Mailbox address
            </label>
            <input
              id={`${titleId}-mailbox`}
              className={css.mailbox}
              value={values.mailboxAddress}
              disabled={!snapshot.writable || busy}
              aria-describedby={`${titleId}-mailbox-help`}
              onChange={(event) => {
                setDraft((current) => ({
                  ...current,
            ...
            />
            <p id={`${titleId}-mailbox-help`} className={css.hint}>
              {snapshot.writable
                ? "This address is the Agent's one fixed mailbox."
                : "Mailbox settings are read-only in this DSH profile."}
            </p>
          </div>
          <div className={css.field}>
            <label className={css.mailboxLabel} htmlFor={`${titleId}-endpoint`}>
              MCP endpoint
            </label>
            <input
              id={`${titleId}-endpoint`}
              ...
              value={values.upstreamEndpoint}
              disabled={!snapshot.writable || busy}
              aria-describedby={`${titleId}-endpoint-help`}
              onChange={(event) => {
                setDraft((current) => ({
                  ...current,
                  upstreamEndpoint: event.target.value,
                }));
                setSaveError(undefined);
              }}
            />
            <p id={`${titleId}-endpoint-help`} className={css.hint}>
              Absolute HTTPS URL for the upstream mail MCP.
            </p>
          </div>
          <div className={css.field}>
            <label className={css.mailboxLabel} htmlFor={`${titleId}-issuer`}>
              OAuth issuer
            </label>
            <input
              id={`${titleId}-issuer`}
              ...
              value={values.oauthAuthorizationServer}
              disabled={!snapshot.writable || busy}
              aria-describedby={`${titleId}-issuer-help`}
              onChange={(event) => {
                setDraft((current) => ({
                  ...current,
                  oauthAuthorizationServer: event.target.value,
                }));
                setSaveError(undefined);
              }}
            />
            <p id={`${titleId}-issuer-help`} className={css.hint}>
              Absolute HTTPS authorization-server issuer.
            </p>
          </div>
          <div className={css.field}>
            <label className={css.mailboxLabel} htmlFor={`${titleId}-callback`}>
              OAuth callback URL
            </label>
            <input
              id={`${titleId}-callback`}
              ...
              value={values.oauthCallbackUrl}
              disabled={!snapshot.writable || busy}
              aria-describedby={`${titleId}-callback-help`}
              onChange={(event) => {
                setDraft((current) => ({
                  ...current,
            ...
            />
            <p id={`${titleId}-callback-help`} className={css.hint}>
              HTTPS or loopback HTTP; keep the `/oauth/dsh-mail/callback` path.
            </p>
          </div>
          {saveError ? (
            <output className={`${css.hint} ${css.error}`}>{saveError}</output>
          ) : null}
          <output
            className={`${css.status}${status.state === "failed" ? ` ${css.error}` : connected ? ` ${css.success}` : ""}`}
          ...
          <div className={css.actions}>
            ...
            {connected ? (
              <button
                type="button"
                className={css.secondary}
                disabled={busy || dirty || !saved.mailboxAddress}
...
function useScopeSnapshot(
  ...
  const getSnapshot = useMemo(() => () => snapshotValue(scope), [scope]);
