import { createRuntime, type RuntimeOptions, type ServerDefinition } from "mcporter";
import {
  GUION_MCP_ENDPOINT,
  MCP_CALL_TIMEOUT_MS,
  MCP_SERVER_NAME,
...
export type RuntimeFactory = (options: RuntimeOptions) => Promise<McpRuntimeLike>;
...
type UpstreamErrorCategory = "upstream-auth-rejected" | "upstream-mailbox-not-found" | "upstream-tool-unavailable" | "upstream-rejected";
...
function upstreamErrorCategory(value: Record<string, unknown>): UpstreamErrorCategory {
  const text = textContent(value.content);
  if (/\b(401|403)\b|unauthori[sz]ed|forbidden|invalid[_ -]?token|access[_ -]?denied|invalid_target/iu.test(text)) {
    return "upstream-auth-rejected";
  }
  if (/mailbox.{0,80}(not found|unknown|does not exist|unavailable)|(?:not found|unknown|does not exist).{0,80}mailbox/iu.test(text)) {
    return "upstream-mailbox-not-found";
  }
  if (/unknown tool|tool not found|method not found|unsupported tool/iu.test(text)) {
...
function asJson(value: unknown, depth = 0): unknown {
  ...
  if (Array.isArray(value)) return value.slice(0, 100).map((entry) => asJson(entry, depth + 1));
  if (typeof value === "object") {
    const output: Record<string, unknown> = {};
    for (const [key, entry] of Object.entries(value).slice(0, 200)) {
      if (/token|secret|password|authorization|client_secret|refresh/i.test(key)) {
        output[key] = "[redacted]";
      } else {
...
export function normalizeMcpResult(value: unknown): unknown {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (record.structuredContent !== undefined) return asJson(record.structuredContent);
    const text = textContent(record.content);
    if (text) {
      try { return asJson(JSON.parse(text)); } catch { return text.slice(0, 64_000); }
    }
  }
  return asJson(value);
}
...
export class GuionMcpClient {
  ...
  constructor(options: GuionMcpClientOptions = {}) {
    this.endpoint = validateHttpsUrl(options.endpoint ?? GUION_MCP_ENDPOINT, "upstreamEndpoint");
    this.runtimeFactory = options.runtimeFactory ?? (async (runtimeOptions) => await createRuntime(runtimeOptions));
    this.clientInfo = options.clientInfo ?? { name: "dsh-mail", version: "0.1.0" };
  }
  ...
  async call(toolName: UpstreamToolName, args: Record<string, unknown>, accessToken: string, signal?: AbortSignal): Promise<unknown> {
    if (!UPSTREAM_TOOL_NAMES.includes(toolName)) throw new Error("upstream-tool-not-allowed");
    ...
    if (this.registeredToken !== accessToken) {
      if (this.registeredToken !== undefined) await runtime.close(MCP_SERVER_NAME).catch(() => undefined);
      runtime.registerDefinition(this.definition(accessToken), { overwrite: true });
  ...
  private async awaitCancellation(call: Promise<unknown>, runtime: McpRuntimeLike, signal?: AbortSignal): Promise<unknown> {
