...
export function validateOAuthAuthorizationServerUrl(
  value: unknown,
  field = "oauthAuthorizationServer",
): string {
  const validated = validateHttpsUrl(value, field);
  if (new URL(validated).search) {
    throw new Error(
      `dsh-mail ${field} must be an absolute HTTPS URL without a query or fragment.`,
    );
  }
  return validated;
}
...
export function validateOAuthCallbackUrl(
  ...
  if (
    ...
    throw new Error(
      `dsh-mail ${field} must be an absolute HTTPS URL or an HTTP loopback URL.`,
    );
  }
  let url: URL;
  try {
  ...
  const safeHttpLoopback =
    url.protocol === "http:" && isLoopbackHost(url.hostname);
  if (
    (!safeHttps && !safeHttpLoopback) ||
    url.username ||
    url.password ||
    url.hash ||
...
export function resolveMailConfig(
  config: MailConnectionConfig | null | undefined = {},
): ResolvedMailConfig {
  if (!config || typeof config !== "object" || Array.isArray(config)) {
    throw new Error("dsh-mail configuration must be an object.");
  }
  const upstreamEndpoint = validateHttpsUrl(
    config.upstreamEndpoint ?? GUION_MCP_ENDPOINT,
    "upstreamEndpoint",
  );
  ...
  const oauthCallbackUrl = validateOAuthCallbackUrl(
    config.oauthCallbackUrl ?? OAUTH_REDIRECT_URI,
  );
  return {
    ...config,
    upstreamEndpoint,
