...
describe("mail tool projections", () => {
  it("registers exactly six closed model capabilities and fixes the mailbox upstream", async () => {
    ...
    const serializedSchemas = JSON.stringify(tools.map((tool) => tool.parameters));
    expect(serializedSchemas).not.toMatch(/mailboxId|cc|bcc|replyAll/);

    await tools.find((tool) => tool.name === MAIL_LIST)!.execute({ folder: "INBOX", limit: 5, page: 2 }, execution());
    await tools.find((tool) => tool.name === MAIL_SEARCH)!.execute({ query: "invoice", folder: "INBOX" }, execution());
    await tools.find((tool) => tool.name === MAIL_READ)!.execute({ emailId: "m-1" }, execution());
    await tools.find((tool) => tool.name === MAIL_READ_THREAD)!.execute({ threadId: "t-1" }, execution());
    await tools.find((tool) => tool.name === MAIL_SEND)!.execute({ to: "person@example.com", subject: "Hi", bodyHtml: "<p>Hello</p>" }, execution());
    await tools.find((tool) => tool.name === MAIL_REPLY)!.execute({
      originalEmailId: "m-1",
    ...
    expect(calls.map(({ tool }) => tool)).toEqual([
      "list_emails", "search_emails", "get_email", "get_thread", "send_email", "send_reply"
    ]);
  ...
  it("ignores a direct mailbox selector and returns a safe provider failure", async () => {
    ...
    await expect(tools[0]!.execute({ limit: 1, mailboxId: "other@example.com" }, execution())).rejects.toThrow("The upstream mail request failed.");
  ...
  it("fails before upstream work when the credential is absent", async () => {
    ...
    await expect(service.call("list_emails", {}, new AbortController().signal)).rejects.toThrow(
  ...
  it("explains when the configured Agent mailbox is unavailable", async () => {
    const tools = createMailToolDefinitions({
      call: async () => { throw new Error("upstream-mailbox-not-found"); }
    });
  ...
  it("uses the documented defaults for the upstream and OAuth connection", async () => {
    ...
    const service = new MailService({
      ...
      credentialStore: {
        read: async () => ({
          ...
          payload: {
            ...
            clientId: "client-1",
            binding: JSON.stringify([GUION_MCP_ENDPOINT, DEFAULT_OAUTH_AUTHORIZATION_SERVER, OAUTH_REDIRECT_URI, undefined, undefined])
          }
        }),
        write: async () => undefined,
        clear: async () => undefined
      ...
      runtimeFactory: async (options) => {
        ...
        return {
          registerDefinition(value) { definition = value; },
    ...
    await expect(service.call("list_emails", {}, new AbortController().signal)).resolves.toEqual({});
    expect(runtimeOptions?.servers).toEqual([]);
    expect(definition?.command).toMatchObject({ kind: "http", url: new URL(GUION_MCP_ENDPOINT) });
    await service.close();
  });
  ...
  it("routes a custom endpoint through OAuth and the capability-local MCP runtime", async () => {
    ...
    const service = new MailService({
      ...
      credentialStore: {
        read: async () => grant ? { kind: "grant" as const, payload: JSON.parse(grant) } : undefined,
        write: async (_ref, value) => { grant = value; },
      ...
      fetcher: async (input, init) => {
        ...
        if (url.pathname === "/register") {
          return new Response(JSON.stringify({ client_id: "custom-client" }), { headers: { "content-type": "application/json" } });
        }
        if (url.pathname === "/token") {
          return new Response(JSON.stringify({ access_token: "custom-token", expires_in: 3600 }), { headers: { "content-type": "application/json" } });
        }
    ...
    await expect(service.callback(new URLSearchParams({ state: start.attemptId!, code: "custom-code" }))).resolves.toMatchObject({ state: "connected" });
    ...
    await expect(service.call("list_emails", { mailboxId: "other@example.com" }, new AbortController().signal)).resolves.toEqual({ ok: true });
    expect(runtimeOptions?.servers).toEqual([]);
    expect(definition?.command).toMatchObject({ kind: "http", url: new URL(endpoint) });
    expect(call).toEqual({ server: "guion-email", tool: "list_emails", args: { mailboxId: "agent@example.com" } });
    await service.close();
  });
});
