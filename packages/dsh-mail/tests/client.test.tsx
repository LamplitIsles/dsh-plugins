...
function scope(
  status: SettingsScopeSnapshot<{
    mailboxAddress?: string;
  }>["status"] = "ready",
): SettingsScope<{ mailboxAddress?: string }> {
  const snapshot: SettingsScopeSnapshot<{ mailboxAddress?: string }> = {
    status,
    value:
      status === "ready" ? { mailboxAddress: "agent@example.com" } : undefined,
    base: undefined,
    user: undefined,
    revision: 1,
    writable: false,
...
function connection(responses: unknown[]): {
...
function editableScope(mutations: unknown[][]): SettingsScope<{
  mailboxAddress?: string;
...
describe("mail settings card", () => {
  ...
  it("opens only the Host-returned authorization URL and keeps token data out of the card", async () => {
    const transport = connection([
      { state: "idle" },
      {
        state: "pending",
        attemptId: "attempt-1",
        authorizationUrl: "https://access.example/authorize?state=1",
      },
    ...
    expect(
      renderer!.root.findByProps({ "data-connection-state": "idle" }),
    ).toBeTruthy();
    await act(async () => {
      findAction(renderer!, "connect").props.onClick();
      await Promise.resolve();
    });
  ...
  it("renders a pending cancellation action and can retry a failed state", async () => {
    ...
    act(() => {
      renderer = create(
        <MailSettingsCard scope={scope()} connection={transport.handle} />,
      );
    });
    await act(async () => {
      await Promise.resolve();
    ...
    await act(async () => {
      findAction(renderer!, "cancel").props.onClick();
      await Promise.resolve();
    });
    expect(
      renderer!.root.findByProps({ "data-connection-state": "cancelled" }),
    ).toBeTruthy();
    await act(async () => {
      findAction(renderer!, "connect").props.onClick();
      await Promise.resolve();
    ...
    expect(
      renderer!.root.findByProps({ "data-connection-state": "failed" }),
    ).toBeTruthy();
  });
  ...
  it("allows a connected mailbox to be reauthorized with a fresh grant", async () => {
    ...
    expect(
      renderer!.root.findByProps({ "data-connection-state": "connected" }),
    ).toBeTruthy();
    await act(async () => {
      findAction(renderer!, "reconnect").props.onClick();
      await Promise.resolve();
    });
  ...
  it("edits and saves MCP, issuer, and callback connection settings together", async () => {
    ...
    act(() => {
      renderer = create(
        <MailSettingsCard
          scope={editableScope(mutations)}
          connection={connection([{ state: "idle" }]).handle}
        />,
      );
    });
    await act(async () => {
    ...
    const labels = renderer!.root
      ...
      .map((node) => textChildren(node.children));
    ...
    const endpoint = renderer!.root.find(
      (node) =>
        typeof node.props.id === "string" &&
        node.props.id.endsWith("-endpoint"),
    );
    act(() => {
      endpoint.props.onChange({
        target: { value: "https://mail.changed.test/mcp" },
      });
    });
    await act(async () => {
      findAction(renderer!, "save").props.onClick();
      await Promise.resolve();
    });
    expect(mutations).toEqual([
      [
        { op: "set", path: ["mailboxAddress"], value: "agent@example.com" },
        {
          ...
          value: "https://mail.changed.test/mcp",
        },
        {
          op: "set",
          path: ["oauthAuthorizationServer"],
          value: "https://login.example.test",
        },
        {
          op: "set",
          path: ["oauthCallbackUrl"],
          value: "http://127.0.0.1:3080/oauth/dsh-mail/callback",
        },
      ],
    ]);
  });
