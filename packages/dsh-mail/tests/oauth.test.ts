...
function store() {
  ...
  return {
    ...
    get writes() { return writes; },
      credentialStore: {
        read: async () => value ? { kind: "grant" as const, payload: JSON.parse(value) } : undefined,
      write: async (_ref: string, next: string) => { value = next; writes += 1; },
...
describe("Host-owned OAuth PKCE lifecycle", () => {
  it("does not reuse a grant issued for different saved connection settings", async () => {
    const saved = store();
    await saved.credentialStore.write("grant", JSON.stringify({
    ...
    const manager = new OAuthManager({
      ...
      metadata: { authorization_endpoint: "https://access.example/authorize", token_endpoint: "https://access.example/token" },
      clientId: "client-1",
      grantBinding: "new-connection"
    });

    await expect(manager.accessToken()).rejects.toThrow("Mail is not connected");
  });
  ...
  it("stays pending while the callback token exchange is in flight", async () => {
    ...
    const callback = manager.callback(new URLSearchParams({ state: start.attemptId!, code: "code" }));

    await expect(manager.status()).resolves.toMatchObject({ state: "pending" });

    completeExchange?.(response({ access_token: "access-1", expires_in: 3600 }));
  ...
  it("does not report a transient authorization failure for an overlapping callback replay", async () => {
    ...
    const start = await manager.start();
    const firstCallback = manager.callback(new URLSearchParams({ state: start.attemptId!, code: "code" }));
    const overlappingReplay = manager.callback(new URLSearchParams({ state: start.attemptId!, code: "code" }));
    completeExchange?.(response({ access_token: "access-1", expires_in: 3600 }));
    const [firstResult, replayResult] = await Promise.all([firstCallback, overlappingReplay]);
  ...
  it("uses dynamic registration, consumes state once, stores a grant, and refreshes it", async () => {
    ...
    const manager = new OAuthManager({
      ...
      fetcher: async (input, init) => {
        ...
        if (String(input).endsWith("/register")) return response({ client_id: "client-1" });
        if (String(input).endsWith("/token")) {
          const form = new URLSearchParams(String(init?.body ?? ""));
          if (form.get("grant_type") === "authorization_code") return response({ access_token: "access-1", refresh_token: "refresh-1", expires_in: 30 });
    ...
    expect(authorization.searchParams.get("redirect_uri")).toBe(OAUTH_REDIRECT_URI);
    expect(authorization.searchParams.get("resource")).toBe("https://mail.guion.io");
    expect(authorization.searchParams.get("code_challenge_method")).toBe("S256");
    expect(authorization.searchParams.get("client_id")).toBe("client-1");
    expect(requests[0]?.init?.method).toBe("POST");

    const invalid = await manager.callback(new URLSearchParams({ state: start.attemptId!, code: "bad" }));
    expect(invalid.state).toBe("connected");
    expect(saved.writes).toBe(1);
    const authorizationCodeForm = new URLSearchParams(String(requests[1]?.init?.body ?? ""));
    ...
    const replay = await manager.callback(new URLSearchParams({ state: start.attemptId!, code: "bad" }));
    ...
    const refreshForm = new URLSearchParams(String(requests[2]?.init?.body ?? ""));
  ...
  it("uses a configured issuer and callback for discovery, registration, authorization, and token exchange", async () => {
    ...
    const manager = new OAuthManager({
      ...
      fetcher: async (input, init) => {
        ...
        if (url.pathname === "/tenant/register") return response({ client_id: "custom-client" });
        if (url.pathname === "/tenant/token") return response({ access_token: "custom-access", expires_in: 3600 });
        throw new Error(`unexpected request ${url}`);
      }
    });

    const start = await manager.start();
    expect(requests[0]?.input).toBe(`${new URL(authorizationServer).origin}/.well-known/oauth-authorization-server/tenant`);
    const registration = JSON.parse(String(requests[1]?.init?.body ?? "{}")) as { redirect_uris?: string[] };
    expect(registration.redirect_uris).toEqual([redirectUri]);
    const authorization = new URL(start.authorizationUrl!);
    expect(authorization.searchParams.get("redirect_uri")).toBe(redirectUri);
    expect(authorization.searchParams.get("resource")).toBe("https://mail.custom.test");

    await expect(manager.callback(new URLSearchParams({ state: start.attemptId!, code: "custom-code" }))).resolves.toMatchObject({ state: "connected" });
    const tokenForm = new URLSearchParams(String(requests[2]?.init?.body ?? ""));
  ...
  it("uses the path-aware OIDC well-known URL when OAuth metadata is unavailable", async () => {
    ...
    const manager = new OAuthManager({
      ...
      fetcher: async (input) => {
        ...
        const url = new URL(String(input));
        if (url.pathname === "/.well-known/oauth-authorization-server/tenant") return response({}, false);
        ...
        if (url.pathname === "/tenant/register") return response({ client_id: "oidc-client" });
        throw new Error(`unexpected request ${url}`);
      }
    });
  ...
  it("does not store invalid, cancelled, or expired callbacks", async () => {
    ...
    const denied = await manager.callback(new URLSearchParams({ state: invalidStart.attemptId!, error: "access_denied" }));
    expect(denied.state).toBe("failed");
    expect(denied.message).toBe("Cloudflare Access rejected authorization (access_denied). Retry from DSH Settings.");
    ...
    expect((await manager.callback(new URLSearchParams({ state: cancelledStart.attemptId!, code: "late" }))).state).toBe("failed");
    expect(saved.writes).toBe(0);

    const expiredStart = await manager.start();
    now += 5 * 60_000 + 1;
    expect((await manager.callback(new URLSearchParams({ state: expiredStart.attemptId!, code: "late" }))).state).toBe("failed");
    expect(saved.writes).toBe(0);
  });
  ...
  it("reports a safe token-exchange rejection without storing a grant", async () => {
    ...
    const start = await manager.start();
    const result = await manager.callback(new URLSearchParams({ state: start.attemptId!, code: "code" }));
    expect(result.message).toBe("Cloudflare Access rejected the token exchange (HTTP 400). Retry from DSH Settings.");
    expect(saved.writes).toBe(0);
  });
  ...
  it("identifies a safe authorization setup stage", async () => {
    const manager = new OAuthManager({
      ...
      fetcher: async () => { throw new Error("provider detail must not surface"); }
    });

    await expect(manager.start()).resolves.toMatchObject({
      state: "failed",
      message: "Mail authorization setup failed during client registration. Retry from DSH Settings."
    });
  });
});
