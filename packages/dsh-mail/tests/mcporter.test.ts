...
function fakeRuntime(call: () => Promise<unknown>) {
  ...
  return {
    ...
    closed,
    runtime: {
      registerDefinition(definition: unknown) {
...
describe("capability-local MCPorter client", () => {
  it("starts with no ambient servers and registers only the Guion mail tool allowlist", async () => {
    ...
    const fake = fakeRuntime(async () => ({
      content: [
        { type: "text", text: JSON.stringify({ messages: [{ id: "m-1" }] }) },
      ],
    }));
    ...
    await expect(
      client.call(
        "list_emails",
        { mailboxId: "agent@example.com" },
        "access-token",
      ),
    ).resolves.toEqual({ messages: [{ id: "m-1" }] });
  ...
  it("classifies an upstream authorization rejection without passing through details", async () => {
    const fake = fakeRuntime(async () => ({
      isError: true,
      content: [
        { type: "text", text: "401 Unauthorized: secret provider details" },
      ],
    }));
    const client = new GuionMcpClient({
      runtimeFactory: async () => fake.runtime,
    ...
    await expect(
      client.call("get_email", { emailId: "m" }, "token"),
    ).rejects.toThrow("upstream-auth-rejected");
  });

  it("classifies a missing configured mailbox without passing through provider details", async () => {
    const fake = fakeRuntime(async () => ({
      isError: true,
      content: [{ type: "text", text: "Mailbox agent@example.com not found" }],
    }));
    const client = new GuionMcpClient({
      runtimeFactory: async () => fake.runtime,
    });
    await expect(
      client.call("list_emails", { mailboxId: "agent@example.com" }, "token"),
    ).rejects.toThrow("upstream-mailbox-not-found");
  });
});
