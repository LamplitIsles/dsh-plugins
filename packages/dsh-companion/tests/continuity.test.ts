...
function event(
  ...
  return {
    ...
    seq,
    time: seq * 100,
    data: { compactionId, turn: null, ...data },
  } as never;
}

function match(raw: unknown, role: "start" | "update") {
  return { event: raw, role, location: { kind: "session" } } as never;
}
...
describe("Companion context capacity", () => {
  it("prefers projected pressure and caps a rounded percentage", () => {
    ...
    ).toEqual({ usedTokens: 123, contextWindow: 100, percentage: 100 });
  });

  it("falls back to the provider anchor only when projected pressure is absent", () => {
    expect(
      resolveContextCapacity({ pressureTokens: 51, contextWindow: 100 }),
    ).toMatchObject({ usedTokens: 51, percentage: 51 });
    expect(
      resolveContextCapacity({
        pressureTokens: 51,
  ...
  it("hides incomplete or non-positive telemetry", () => {
    expect(
      resolveContextCapacity({ pressureTokens: 0, contextWindow: 100 }),
    ).toBeUndefined();
    expect(
      resolveContextCapacity({
        pressureTokens: 20,
    ...
    expect(
      resolveContextCapacity({ pressureTokens: 20, contextWindow: 0 }),
    ).toBeUndefined();
    expect(
      resolveContextCapacity({
        pressureTokens: Number.NaN,
        contextWindow: 100,
      }),
    ).toBeUndefined();
  });
...
describe("Companion compaction lifecycle", () => {
  it("disposes both registry contributions so a fresh registration can reuse their keys", () => {
    ...
    const events = {
      register(definition: { kind: string }) {
        if (eventDefinitions.has(definition.kind))
          throw new Error("duplicate event key");
        eventDefinitions.set(definition.kind, definition);
        return () => {
          if (eventDefinitions.get(definition.kind) === definition)
            eventDefinitions.delete(definition.kind);
        };
      },
    };
    const views = {
      register(definition: { target: string }) {
        if (viewDefinitions.has(definition.target))
          throw new Error("duplicate view key");
        viewDefinitions.set(definition.target, definition);
        return () => {
          if (viewDefinitions.get(definition.target) === definition)
            viewDefinitions.delete(definition.target);
        };
      },
    };
    ...
    expect(eventDefinitions.has("dsh-companion:compaction-lifecycle")).toBe(
      true,
    );
  ...
  it("turns a public start/end pair into one ordered view node", () => {
    ...
    const state = compactionLifecycleDefinition.start(
      {} as never,
      match(start, "start"),
      {} as never,
    );
    const end = event("compaction/end", 12);
    const next = compactionLifecycleDefinition.update(
      { state } as never,
      match(end, "update"),
    );
    ...
    const node = compactionLifecycleDefinition.buildViewNode!({
      key: "dsh-companion:compaction-lifecycle:compact-1",
      kind: compactionLifecycleDefinition.kind,
      id: "compact-1",
      matches: [],
      start: undefined,
      state: next,
    ...
    expect(node).toMatchObject({
      target: "dsh-companion:continuity",
    ...
    const first = builder.replace({
      nodes: [node as CompactionLifecycleNode],
      timeline,
    });
    const replay = builder.apply({
      upserts: [node as CompactionLifecycleNode],
      timeline,
    });
    ...
    expect(
      compactionLifecycleDefinition.buildViewNode!({
        state: undefined,
      } as never),
    ).toBeNull();
  });
  ...
  it("keeps failed wording technical-detail free and ignores private summaries", () => {
    const startState = compactionLifecycleDefinition.start(
      {} as never,
      match(event("compaction/start", 1, "failed"), "start"),
      {} as never,
    );
    const failed = compactionLifecycleDefinition.update(
      { state: startState } as never,
      match(
        event("compaction/end", 2, "failed", { error: "private stack trace" }),
        "update",
      ),
    );
    expect(failed.status).toBe("failed");
    const completed: CompactionLifecycleState = {
      compactionId: "safe",
      status: "complete",
      startSeq: 3,
      startedAt: 300,
    ...
    const records = projectContinuityRecords(
      { lifecycles: [completed, completed] },
      [
        {
          kind: "compaction",
          id: "safe",
          seq: 5,
          shadowedTokenCount: 18_432,
    ...
    expect(
      projectContinuityRecords({ lifecycles: [completed] }, [
        {
          kind: "compaction",
          id: "safe",
          data: { seq: 5, shadowedTokenCount: 18_432, summary: "PRIVATE" },
        },
      ]),
    ).toEqual([expect.objectContaining({ text: "已整理对话" })]);
  ...
  it("inserts one durable completion record at its compaction anchor", () => {
    const lifecycle: CompanionContinuitySnapshot = {
      lifecycles: [
        {
          compactionId: "one",
          status: "complete",
    ...
    const snapshot = {
      nodes: [
        ...
        },
        {
          ...
          seq: 4,
          time: 4,
          blocks: [{ kind: "text", text: "现在" }],
        },
      ],
    };
    const result = projectConversation(snapshot, true, lifecycle);
    expect(
      result.items.filter((item) => item.kind === "continuity"),
    ).toHaveLength(1);
