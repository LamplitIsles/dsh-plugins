...
const chat = (entries: readonly [string, string, string][] = []) => ({
  ...
  nodes: new Map(
    entries.map(([key, rpcId, text], index) => [
      key,
      {
        key,
        kind: "user",
...
function outgoingCount(snapshot: unknown): number {
  return projectConversation(snapshot).items.filter(
...
describe("SubmissionHandoff", () => {
  ...
  it("keeps an idle transcript echo until Chat publishes its durable node", () => {
    ...
    const optimistic = handoff.merge(
      { sessionId: "session-1", pendingSubmissions: pending, queue: [] },
      chat(),
    );
    handoff.retire("req-idle", "observed");
    const betweenStores = handoff.merge(
      { sessionId: "session-1", pendingSubmissions: [], queue: [] },
      chat(),
    );
    const durable = handoff.merge(
      { sessionId: "session-1", pendingSubmissions: [], queue: [] },
      chat([["durable-idle", "req-idle", "直接发送"]]),
    );

    expect([optimistic, betweenStores, durable].map(outgoingCount)).toEqual([
      1, 1, 1,
    ]);
  });
  ...
  it("keeps a submission visible while separately published Session and Chat snapshots hand it off", () => {
    ...
    const projected = [
      ...
      handoff.merge(
        { sessionId: "session-1", pendingSubmissions: pending, queue: queued },
        chat(),
      ),
      handoff.merge(
        { sessionId: "session-1", pendingSubmissions: [], queue: queued },
        chat(),
      ),
      handoff.merge(
        { sessionId: "session-1", pendingSubmissions: [], queue: [] },
    ...
    expect(
      projected.map((snapshot) =>
        projectConversation(snapshot).messageUnits.map((unit) => unit.id),
      ),
    ).toEqual([
      ["submission:req-1"],
      ["submission:req-1"],
    ...
    expect(
      projected.map((snapshot) => {
        const item = projectConversation(snapshot).items.find(
          (candidate) =>
            candidate.kind === "text" &&
            candidate.messageKey === "submission:req-1",
        );
        return item?.kind === "text" ? item.waitsForCurrentReply : undefined;
      }),
    ).toEqual([false, false, false, false, undefined]);
  });
  ...
  it("retains queued admission through queue handoff after the local echo retires", () => {
    ...
    const queued = [
      {
        id: "queue-active",
        messageId: "queue-active",
        placement: "queued",
        rpcId: "req-active",
        content: [{ type: "text", text: "等我说完" }],
        preview: "等我说完",
        text: "等我说完",
      },
    ];

    const beforeRetirement = handoff.merge(
      { sessionId: "session-1", pendingSubmissions: pending, queue: queued },
      chat(),
    );
    handoff.retire("req-active", "observed");
    const afterRetirement = handoff.merge(
      { sessionId: "session-1", pendingSubmissions: [], queue: queued },
      chat(),
    );
    ...
    for (const snapshot of [beforeRetirement, afterRetirement]) {
      const item = projectConversation(snapshot).items.find(
        (candidate) =>
          candidate.kind === "text" &&
          candidate.messageKey === "submission:req-active",
      );
      expect(
        item?.kind === "text" ? item.waitsForCurrentReply : undefined,
      ).toBe(true);
    }
  });
