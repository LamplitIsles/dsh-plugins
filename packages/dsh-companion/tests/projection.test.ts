...
describe("chat projection", () => {
  it("shows human text, assistant text/media/voice and pending rows while hiding tools", () => {
    const result = projectConversation(
      {
        nodes: [
          ...
          },
          {
            ...
            seq: 2,
            time: 2,
            blocks: [{ kind: "text", text: "[[tts:text]]慢慢说[[/tts:text]]" }],
          },
          {
            kind: "tool-result",
            seq: 3,
        ...
        queue: [{ id: "q", messageId: "m", text: "排队" }],
        running: true,
        openState: "open",
      },
      true,
    ...
    expect(result.items.find((item) => item.kind === "voice")).toMatchObject({
      text: "慢慢说",
    });
  ...
  it("projects one stable message unit for ordered text and image contributions", () => {
    ...
    expect(result.messageUnits[0]?.items.map((item) => item.kind)).toEqual([
      "text",
      "image",
      "image",
    ]);
    expect(
      result.messageUnits[0]?.items
  ...
  it("keeps direct assistant text when an image block shares the contribution", () => {
    ...
    expect(result.messageUnits[0]?.items[0]).toMatchObject({
      kind: "text",
      text: "先说一句",
    });
  });
  ...
  it("preserves a reader anchor and offers new-message affordance", () => {
    expect(
      scrollPlan({ scrollTop: 20, scrollHeight: 1000, clientHeight: 600 }),
    ).toMatchObject({
      follow: false,
      preserveAnchor: true,
      showNewMessageAffordance: true,
    });
    expect(
      scrollPlan({ scrollTop: 395, scrollHeight: 1000, clientHeight: 600 }),
    ...
    ).toMatchObject({ preserveAnchor: true, follow: false });
  });
  ...
  it("projects alpha pending submissions as normal outgoing rows and atomically swaps by request id", () => {
      ...
      pendingSubmissions: [
        {
          ...
          text: "立即显示",
          images: [
            { previewUrl: "blob:one", name: "one.png" },
            { previewUrl: "blob:two", name: "two.png" },
          ],
        },
      ],
    });
    expect(pending.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "submission:req-1:text",
          side: "outgoing",
          text: "立即显示",
        }),
        expect.objectContaining({
          id: "submission:req-1:image:0",
          side: "outgoing",
          previewUrl: "blob:one",
        }),
        expect.objectContaining({
          id: "submission:req-1:image:1",
          side: "outgoing",
          previewUrl: "blob:two",
        }),
      ]),
    ...
    expect(
      durable.items.filter(
        (item) => item.kind === "text" && item.side === "outgoing",
      ),
    ).toHaveLength(1);
    expect(durable.messageUnits).toContainEqual(
      expect.objectContaining({ id: "submission:req-1", side: "outgoing" }),
    );
    expect(durable.items).toContainEqual(
      expect.objectContaining({
        id: "durable",
        messageKey: "submission:req-1",
      }),
    );
    expect(
      durable.items.some((item) => item.id.startsWith("submission:req-1")),
    ).toBe(false);
  });

  it("hands a pending submission to its observed queue row without duplicate images", () => {
    const image = {
      attachmentId: "queued-image",
      mediaType: "image/png",
      name: "queued.png",
    };
    const result = projectConversation({
      pendingSubmissions: [
        {
          ...
          time: 10,
          text: "稍后发送",
          images: [{ previewUrl: "blob:queued", name: "queued.png" }],
        },
      ],
      queue: [
        {
    ...
    expect(result.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "pending:queue-1",
          messageKey: "submission:req-queued",
          kind: "text",
          side: "outgoing",
          text: "稍后发送",
          pending: true,
    ...
    expect(result.messageUnits).toContainEqual(
      expect.objectContaining({ id: "submission:req-queued", pending: true }),
    );
    expect(
      result.items.some((item) => item.id.startsWith("submission:req-queued")),
    ).toBe(false);
    expect(
      result.items.filter(
        (item) => item.kind === "image" && item.side === "outgoing",
      ),
    ).toHaveLength(1);
  });
  ...
  it("keeps one correlated unit across steering admission and ignores a stale queue duplicate", () => {
          ...
          [
            "steering-key",
            {
              ...
              data: {
                seq: 1,
                messageId: "message-1",
                source: { kind: "user", rpcId: "req-steering" },
                content: [{ type: "text", text: "正在回复时发送" }],
              },
            },
          ],
        ]),
    ...
    expect(steering.messageUnits[0]).toMatchObject({
      id: "submission:req-steering",
      side: "outgoing",
    });
    expect(
      steering.items.filter(
        (item) => item.kind === "text" && item.side === "outgoing",
      ),
    ).toHaveLength(1);
    expect(steering.items[0]).toMatchObject({
      id: "steering-key",
      projectionKey: "steering-key",
      messageKey: "submission:req-steering",
    });
    ...
    expect(durable.messageUnits[0]).toMatchObject({
      id: "submission:req-steering",
      side: "outgoing",
    });
    expect(
  ...
  it("keeps one keyed ImageGen row while a call settles", () => {
    ...
    const ready = projectConversation({
      nodes: [
        {
          kind: "tool-result",
          seq: 1,
    ...
    const replay = projectConversation({
      nodes: [
        {
          kind: "tool-call",
          seq: 1,
    ...
    expect(running.items[0]).toMatchObject({
      id: "imagegen:call-1",
      projectionKey: "imagegen:call-1",
      state: "running",
    });
    expect(ready.items[0]).toMatchObject({
      id: "imagegen:call-1:att-1",
      projectionKey: "imagegen:call-1",
      state: "ready",
    });
  ...
  it("normalizes keyed DSH Chat nodes, including admitted steering and invisible compaction", () => {
    ...
    const nodes = new Map<string, unknown>([
      [
        "user-key",
        {
          ...
          kind: "user",
          visibility: "visible",
          data: { seq: 1, time: 1, content: [{ type: "text", text: "你好" }] },
        },
      ],
      [
        "assistant-key",
      ...
      [
        "compaction-key",
        {
          ...
          kind: "compaction",
          visibility: "visible",
          data: { seq: 5, time: 5, summary: "模型专用摘要" },
        },
      ],
    ]);
    const result = projectConversation({
    ...
    expect(
      result.items.some(
        (item) =>
          item.id === "pending:queued-1" ||
          (item.kind === "text" && item.text === "模型专用摘要"),
      ),
    ).toBe(false);
      ...
      nodes: [
        {
          ...
          seq: 1,
          time: 1,
          content: [{ type: "text", text: "我今天有点累" }],
        },
        {
          kind: "compaction",
          seq: 2,
          time: 2,
          summary: "internal continuity checkpoint",
        },
        {
          kind: "assistant",
          seq: 3,
          time: 3,
          blocks: [{ kind: "text", text: "那我们慢一点。" }],
        },
      ],
    });
    expect(
      result.items.map((item) =>
        item.kind === "text" ? [item.side, item.text] : item.kind,
      ),
    ).toEqual([
      ["outgoing", "我今天有点累"],
      ["incoming", "那我们慢一点。"],
  ...
  it("shows only finalized assistant text and always hides reasoning", () => {
    const finalized = projectConversation({
      nodes: [
        {
          kind: "assistant",
          seq: 1,
    ...
    const streamingNode = projectConversation({
      nodes: [
        {
          kind: "assistant",
          seq: 3,
          streaming: true,
          blocks: [{ kind: "text", text: "还没说完" }],
        },
      ],
    });

    expect(finalized.items).toEqual([
      expect.objectContaining({ kind: "text", text: "我在这里。" }),
    ]);
  ...
  it("keeps a queue row only until its keyed steering node arrives", () => {
    ...
    expect(projectConversation(queued).items).toContainEqual(
      expect.objectContaining({ id: "pending:queued-1", pending: true }),
    );
    expect(projectConversation(admitted).items).toContainEqual(
      expect.objectContaining({ id: "steering-key", text: "补充一句" }),
    );
    expect(
      projectConversation(admitted).items.some(
        (item) => item.id === "pending:queued-1",
      ),
    ).toBe(false);
  });
  ...
  it("reports a rejected stop as a stop failure instead of a send failure", () => {
    ...
    expect(rejectedSend.items).toContainEqual(
      expect.objectContaining({ text: "这条消息没发出去，可以再试一次。" }),
    );
  ...
  it("uses the live connection observable even while a Session snapshot remains mounted", () => {
    const sessionSnapshot = {
      nodes: [{ kind: "assistant", seq: 1, text: "仍在这里" }],
      openState: "open",
      running: false,
    };
    expect(projectConversation(sessionSnapshot, false).status).toBe(
      "reconnecting",
    );
