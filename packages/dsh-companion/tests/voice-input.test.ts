...
describe("Companion voice admission", () => {
  it("derives exact raw boundaries from a parameterized Data URL prefix", () => {
    ...
    expect(voiceDataUrlByteLength(mediaType, maxRawBytes!)).toBeLessThanOrEqual(
      MAX_VOICE_DATA_URL_BYTES,
    );
    expect(isVoiceAudioWithinDataUrlLimit(mediaType, maxRawBytes!)).toBe(true);
    expect(voiceDataUrlByteLength(mediaType, maxRawBytes! + 1)).toBeGreaterThan(
      MAX_VOICE_DATA_URL_BYTES,
    );
    expect(isVoiceAudioWithinDataUrlLimit(mediaType, maxRawBytes! + 1)).toBe(
      false,
    );
  });
  ...
  it("applies the same boundary to a browser Blob", () => {
    ...
    expect(validateVoiceRecording(exact)).toMatchObject({
      mediaType,
      bytes: maxRawBytes,
    });
    const next = new Blob([new Uint8Array(maxRawBytes + 1)], {
      type: mediaType,
    });
    expect(() => validateVoiceRecording(next)).toThrowError(
      VoiceRecordingError,
    );
  });

  it("chooses only a provider-compatible advertised MIME type", () => {
    expect(selectVoiceMimeType(FakeRecorder as never)).toBe(
  ...
  it("records once, stops tracks, and returns an ephemeral bounded Blob", async () => {
    ...
    const controller = new VoiceRecordingController({
      ...
      mediaDevices: {
        getUserMedia: vi.fn<() => Promise<FakeStream>>(async () => stream),
      },
  ...
  it("rejects secure-context, duration, and cumulative byte failures", async () => {
    ...
    class LargeRecorder extends FakeRecorder {
      stop(): void {
        this.ondataavailable?.({
          data: new Blob([new Uint8Array(11)], { type: this.mimeType }),
        });
        this.onstop?.();
      }
    }
    const unavailable = new VoiceRecordingController({
    ...
    const tooLarge = new VoiceRecordingController({
      isSecureContext: true,
      mediaDevices: { getUserMedia: async () => stream },
      mediaRecorder: LargeRecorder as never,
      maxBytes: 10,
    });
    await tooLarge.start();
    await expect(tooLarge.stopAndGet()).rejects.toMatchObject({
      code: "size-limit",
    });
  ...
  it("marks transcribed turns and keeps a raw bracketed expression label", async () => {
    ...
    expect(
      normalizeVoiceTranscription({
        text: "  你好 ",
        sentences: [{ expression: "sad", startMs: 0, endMs: 2, text: "你好" }],
      }),
    ).toEqual({ text: "你好", expression: "sad" });
    expect(formatVoiceTurn({ text: "你好", expression: "sad" })).toBe(
      "🎙️ 你好 [sad]",
    );
    expect(formatVoiceTurn({ text: "你好" })).toBe("🎙️ 你好");
    expect(() => normalizeVoiceTranscription({ text: "" })).toThrowError(
      VoiceRecordingError,
    );
    expect(
      normalizeVoiceTranscription({ text: "你好", expression: "unknown" }),
    ).toEqual({ text: "你好" });
  });
});
