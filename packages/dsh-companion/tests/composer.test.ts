...
import {
  CompanionAdmissionError,
  CompanionPreControllerError,
  submitCompanionInput,
} from "../src/client/admission.js";
import {
...
describe("IME composer", () => {
  it("keeps a one-line minimum, caps long drafts, and enables only internal overflow at the cap", () => {
    expect(resolveComposerHeight(12)).toEqual({
      height: COMPOSER_MIN_HEIGHT,
      scrollable: false,
    });
    expect(resolveComposerHeight(90)).toEqual({
      height: 90,
      scrollable: false,
    });
    expect(resolveComposerHeight(COMPOSER_MAX_HEIGHT + 1)).toEqual({
      height: COMPOSER_MAX_HEIGHT,
      scrollable: true,
    });
    expect(resolveComposerHeight(0, 40, 120)).toEqual({
      height: 40,
      scrollable: false,
    });
  });
  ...
  it("does not submit while composing and submits once after composition", () => {
    ...
    expect(
      shouldSubmitEnter({ key: "Enter", isComposing: true }, state.composing),
    ).toBe(false);
...
it("retains staged settings while clean fields follow a refreshed Host baseline", () => {
  ...
  expect(mergeCleanSettingsDraft(draft, before, refreshed)).toEqual({
    ...refreshed,
    companionName: "Mio!",
  });
  expect(changedSettingsPayload(draft, before)).toEqual({
    companionName: "Mio!",
  });
});
...
describe("Companion alpha Session admission", () => {
  it("registers an echo before prompt and propagates its exact request id", async () => {
    ...
    await submitCompanionInput(
      {
        beginSubmission: (input) => {
          begins.push(input);
          return { requestId: "request-1" as never, abandon: () => undefined };
        },
        prompt: async (...args) => {
          calls.push(args);
    ...
    expect(begins).toEqual([
      expect.objectContaining({ mode: "queue", text: "消息", images: [] }),
    ]);
    expect(calls).toEqual([
      [[{ type: "text", text: "消息" }], "queue", undefined, "request-1"],
    ]);
  });
    ...
    const error = await submitCompanionInput(
      {
        beginSubmission: (input) => {
          ...
          return { requestId: "request-2" as never, abandon: () => undefined };
        },
        prompt: async () => ({
          ok: false,
          error: {
  ...
  it("reports the correlated request id when the Session retires an echo", async () => {
    let retirement:
      | ((value: { reason: "observed"; attachments: [] }) => void)
      | undefined;
    ...
    await submitCompanionInput(
      {
        beginSubmission: (input) => {
          ...
          return {
            requestId: "request-retired" as never,
            abandon: () => undefined,
          };
        },
      ...
      [],
      undefined,
      (requestId, value) => observed.push([requestId, value.reason]),
    );
  ...
  it("keeps other slash-prefixed text on the ordinary prompt path", async () => {
    ...
    expect(prompts).toEqual([
      [[{ type: "text", text: "/不是命令" }], "queue", undefined, "unused"],
    ]);
  });
  ...
  it("does not let /compact carry images", async () => {
    ...
    ).rejects.toBeInstanceOf(CompanionPreControllerError);
...
describe("Companion session opening", () => {
  it("opens a remembered selected session once the alpha list is ready", () => {
    expect(
      companionSessionOpenPlan(true, "workspace-a", "remembered-session"),
    ).toEqual({ kind: "open", sessionId: "remembered-session" });
  });

  it("waits for the workspace list before creating a session", () => {
    expect(
      companionSessionOpenPlan(false, "workspace-a", undefined),
    ).toBeUndefined();
    expect(companionSessionOpenPlan(true, "workspace-a", undefined)).toEqual({
      kind: "create",
