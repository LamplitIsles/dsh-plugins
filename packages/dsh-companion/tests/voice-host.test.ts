...
describe("Companion voice Host RPC", () => {
  it("authorizes the configured workspace/session and passes only decoded audio to optional DSH Speech", async () => {
    ...
    ).resolves.toEqual({ ok: true, value: { available: true } });
    await expect(
      handler()(
        VOICE_TRANSCRIBE_ENDPOINT,
        {
  ...
  it("rejects non-canonical/unsupported/foreign requests before calling DSH Speech", async () => {
    const transcribe = vi.fn<() => Promise<{ text: string }>>(async () => ({
      text: "never",
    }));
    const { host, handler } = hostWith({ transcribe });
    for (const payload of [
      ...
      await expect(
        handler()(VOICE_TRANSCRIBE_ENDPOINT, payload, signal),
  ...
  it("uses the complete parameterized Data URL boundary for decoded RPC audio", async () => {
    ...
    ).resolves.toMatchObject({ ok: true, value: { text: "边界" } });
    await expect(
      handler()(
        VOICE_TRANSCRIBE_ENDPOINT,
        {
  ...
  it("reports optional capability absence and hides provider failures", async () => {
    ...
    ).resolves.toEqual({ ok: true, value: { available: false } });
    await expect(
      handler()(
        VOICE_TRANSCRIBE_ENDPOINT,
        {
    ...
    const result = await failing.handler()(
      VOICE_TRANSCRIBE_ENDPOINT,
      {
        workspaceId: workspace.id,
        sessionId: "session-a",
