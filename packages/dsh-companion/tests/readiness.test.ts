...
describe("Companion startup readiness", () => {
  it("keeps Workspace absence unknown while its list is loading", () => {
    expect(
      resolveWorkspaceReadiness("workspace-a", workspace("pending", "loading")),
    ).toBe("loading");
    expect(
      resolveWorkspaceReadiness(
        "workspace-a",
        workspace("ready", "idle", ["workspace-a"]),
      ),
    ).toBe("ready");
    expect(
      resolveWorkspaceReadiness(
        "workspace-a",
        workspace("ready", "idle", ["workspace-b"]),
      ),
    ).toBe("missing");
    expect(
      resolveWorkspaceReadiness("workspace-a", workspace("ready", "error")),
    ).toBe("error");
  });
