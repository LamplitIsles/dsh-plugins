...
test("Companion stylesheet ownership follows its effect lifecycle", async ({
  page,
}) => {
  await page.goto("/");
  ...
  const sheets = await styles.evaluateAll((elements) =>
  ...
  expect(sheets).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        plugin: CLIENT_PLUGIN_ID,
        sheet: APPLICATION_STYLESHEET_ID,
        css: expect.stringContaining("#dsh-companion"),
      }),
      expect.objectContaining({
        plugin: CLIENT_PLUGIN_ID,
        sheet: SETTINGS_STYLESHEET_ID,
        css: expect.stringContaining("--dsw-alias-label-primary"),
      }),
    ]),
  );
  await expect(
    page.locator("#dsh-companion-styles, #dsh-companion-settings-styles"),
  ).toHaveCount(0);
...
test("fixture has complete media states, accessible overlays, and no duplicate image URLs", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(page.locator(".companion-header")).not.toContainText(
    "把平凡日子折成星星",
  );
  ...
  await expect
    .poll(() =>
      playVoice.evaluate((node) => getComputedStyle(node).backgroundColor),
    )
    .toBe("rgba(0, 0, 0, 0)");
  ...
  await page.evaluate(() => {
    const audio = document.querySelector<HTMLAudioElement>(
      '[data-testid="voice-voice:demo:1:abc"] audio',
    )!;
  ...
  await expect(
    page
      .getByTestId("voice-voice:demo:failed")
  ...
  await expect
    .poll(() =>
      singleImage.locator("img").evaluate((node) => {
  ...
  await expect
    .poll(() => page.evaluate(() => window.__companionFixture?.revoked() ?? 0))
    .toBeGreaterThan(0);
...
test("assembled image preview dismisses from its backdrop and returns focus to the opener", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    .locator(".companion-lightbox-backdrop")
    .click({ position: { x: 6, y: 6 } });
  await expect(dialog).toHaveCount(0);
  await expect(opener).toBeFocused();
});

test("assembled image preview follows the official lightbox visual contract", async ({
  page,
}) => {
  await page.goto("/");
    ...
    .poll(async () => ({
      ...
      image: await image.evaluate((node) => {
        ...
        return {
          ...
          centerDelta:
            Math.abs(centerDelta) < 0.5 ? 0 : Math.round(centerDelta),
        };
      }),
...
test("Pixel 7a browser Back dismisses the assembled image preview and returns focus", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "pixel-7a",
    "browser Back preview coverage runs on the mobile project",
  );
  await page.goto("/");
  const opener = page.getByRole("button", { name: "查看大图：今晚的海" });
  await opener.focus();
...
test("relationship card uses the semantic base surface in both themes", async ({
  page,
}) => {
  await page.goto("/");
...
test("composer grows, caps, and shrinks without document overflow", async ({
  page,
}) => {
  await page.goto("/");
...
test("microphone records once beside context capacity and submits a minimal expression label", async ({
  page,
}) => {
    ...
    class FakeRecorder {
      ...
      stop(): void {
        this.ondataavailable?.({
          data: new Blob(["fixture voice"], { type: this.mimeType }),
        });
        this.onstop?.();
      }
    }
  ...
  await page.evaluate(() =>
    window.__companionFixture?.setCapacity({
      projectedTokens: 100,
      contextWindow: 1_000,
    }),
  );
  ...
  expect(
    children.findIndex((name) => name.includes("companion-microphone")),
  ).toBeLessThan(
    children.findIndex((name) => name.includes("companion-context-meter-wrap")),
  );

  await mic.click();
  await expect(mic).toHaveAccessibleName("结束录音");
  await expect(
    page.getByTestId("companion-voice-recording-status"),
  ).toContainText("正在录音");
  await mic.click();
  await expect(
    page.getByTestId("companion-voice-transcribing-status"),
  ).toBeVisible();
  const textarea = page.getByRole("textbox", { name: "写消息" });
  ...
  await expect(
    page.getByText("🎙️ 来自麦克风的测试消息 [sad]", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByTestId("companion-voice-transcribing-status"),
  ).toHaveCount(0);
...
test("failed microphone transcription preserves text and image drafts without sending", async ({
  page,
}) => {
  await page.addInitScript(() => {
    class FakeRecorder {
  ...
  await expect(mic).toHaveAccessibleName("结束录音");
  await mic.click();
  await expect(page.getByTestId("companion-voice-error-status")).toContainText(
    "语音暂时无法使用",
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__companionFixture?.sendCalls() ?? -1),
    )
    .toBe(0);
});

test("keeps startup neutral until each readiness authority settles", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("还没有设置聊天空间", { exact: true }),
  ).toHaveCount(0);
  await expect(
    page.getByText("这段对话暂时打不开", { exact: true }),
  ).toHaveCount(0);

  await page.evaluate(() =>
    window.__companionFixture?.setReadiness({ workspace: "ready" }),
  );
  await expect(page.getByRole("status", { name: "正在加载" })).toBeVisible();
  await page.evaluate(() =>
    window.__companionFixture?.setReadiness({ relationship: "ready" }),
  );
  await expect(page.getByRole("status", { name: "正在加载" })).toBeVisible();
  await page.evaluate(() =>
    window.__companionFixture?.setReadiness({ session: "ready" }),
  );
  await expect(page.getByRole("textbox", { name: "写消息" })).toBeVisible();

  await page.evaluate(() =>
    window.__companionFixture?.setReadiness({ workspace: "missing" }),
  );
  await expect(
    page.getByRole("heading", { name: "还没有设置聊天空间" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "去 DSH 设置选择聊天空间" }),
  ).toBeVisible();
  await expect(page.locator(".companion-recovery")).toContainText(
    "我们不会替你自动切换。",
  );

  await page.evaluate(() =>
    window.__companionFixture?.setReadiness({
      workspace: "ready",
      relationship: "error",
      session: "loading",
    }),
  ...
  await expect(
    page.getByRole("heading", { name: "关系资料暂时打不开" }),
  ).toBeVisible();
  await expect(
    page.getByText("还没有设置聊天空间", { exact: true }),
  ).toHaveCount(0);
  ...
  await expect(
    page.getByRole("heading", { name: "这段对话暂时打不开" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "重新连接" })).toBeVisible();
});

test("CompanionRoot bridge keeps delayed authorities neutral and exposes only settled outcomes", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("关系资料暂时打不开", { exact: true }),
  ).toHaveCount(0);
  await expect(
    page.getByText("这段对话暂时打不开", { exact: true }),
  ).toHaveCount(0);
  ...
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setWorkspace("ready"),
  );
  await expect(loading).toBeVisible();
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setRelationship("ready"),
  );
  await expect(loading).toBeVisible();
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setSession("ready"),
  );
  await expect(page.getByRole("textbox", { name: "写消息" })).toBeVisible();

  await page.goto("/?bridge=1");
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setWorkspace("missing"),
  );
  await expect(
    page.getByRole("heading", { name: "还没有设置聊天空间" }),
  ).toBeVisible();
  ...
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setRelationship("error"),
  );
  await expect(
    page.getByRole("heading", { name: "关系资料暂时打不开" }),
  ).toBeVisible();
  ...
  await expect(page.getByRole("textbox", { name: "写消息" })).toBeVisible();
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setSession("error"),
  );
  await expect(
    page.getByRole("heading", { name: "这段对话暂时打不开" }),
  ).toBeVisible();
  ...
  await page.goto("/?bridge=1");
  await page.evaluate(() =>
    window.__companionBridgeFixture?.setSettingsUnavailable(),
  );
  await expect(
    page.getByRole("heading", { name: "聊天空间暂时打不开" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "重新连接" })).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "还没有设置聊天空间" }),
...
test("Pixel 7a geometry keeps composer and relationship overlay usable", async ({
  page,
}) => {
...
test("admits photos from the library or desktop paste and sends them as a draft", async ({
  page,
}) => {
  await page.addInitScript(() => {
    const cameraFixture = {
    ...
    const capacitor = ((
      window as unknown as { Capacitor?: Record<string, unknown> }
    ).Capacitor ??= {});
    const headers = Array.isArray(capacitor.PluginHeaders)
      ? capacitor.PluginHeaders
      : [];
    capacitor.PluginHeaders = [
      ...headers,
      { name: "Camera", methods: [{ name: "takePhoto", rtype: "promise" }] },
    ];
    capacitor.nativePromise = (
      ...
      if (plugin !== "Camera" || method !== "takePhoto")
        return Promise.reject(new Error("unexpected-plugin-call"));
      cameraFixture.options = options;
      if (cameraFixture.cancelNoCode)
        return Promise.reject(new Error("User cancelled photos app"));
      if (cameraFixture.cancel)
      ...
      return Promise.resolve({
        ...
        saved: false,
        webPath:
          "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScL9pwAAAABJRU5ErkJggg==",
        metadata: { format: "png" },
      });
    };
    (
      window as unknown as { __cameraFixture?: typeof cameraFixture }
    ).__cameraFixture = cameraFixture;
  });
  await page.goto("/");
  const textarea = page.getByRole("textbox", { name: "写消息" });
  ...
  const png = Buffer.from(
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScL9pwAAAABJRU5ErkJggg==",
    "base64",
  );

  await expect(
    page.getByRole("button", { name: "选择照片；长按拍照" }),
  ).toBeEnabled();
  await page
    .locator("#companion-image-library")
    .setInputFiles({ name: "island.png", mimeType: "image/png", buffer: png });
  ...
  await expect
    ...
    .toBe(1);
  const draftPreview = drafts.getByRole("button", {
    name: "查看大图：island.png",
  });
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__companionFixture?.sendCalls() ?? 0),
    )
    .toBe(1);
  ...
  await expect(
    drafts.getByRole("img", { name: "clipboard.png" }),
  ).toBeVisible();
  await expect
    .poll(() =>
      drafts
  ...
  await picker.dispatchEvent("pointerup", {
    bubbles: true,
    pointerType: "touch",
    pointerId: 9,
  });
  await expect(
    drafts.getByRole("img", { name: "camera-photo.png" }),
  ).toBeVisible();
  await expect(send).toBeEnabled();
  expect(
    await page.evaluate(
      () =>
        (window as unknown as { __cameraFixture?: { options?: unknown } })
          .__cameraFixture?.options,
    ),
  ).toEqual({ saveToGallery: false, includeMetadata: true });

  await page.evaluate(() => {
    (
      window as unknown as { __cameraFixture?: { cancel: boolean } }
    ).__cameraFixture!.cancel = true;
  });
  await picker.dispatchEvent("pointerdown", {
    bubbles: true,
    pointerType: "touch",
  ...
  await page.evaluate(() => {
    const fixture = (
      window as unknown as {
        __cameraFixture?: { cancel: boolean; cancelNoCode: boolean };
      }
    ).__cameraFixture!;
...
test("retries one durable image in place after a transient load failure", async ({
  page,
}) => {
  await page.goto("/");
...
test("keeps a pure-text send row and bubble connected across durable confirmation", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect
    .poll(() => rowHandle!.evaluate((node) => node.isConnected))
    .toBe(true);
  await expect
    .poll(() => bubbleHandle!.evaluate((node) => node.isConnected))
    .toBe(true);
  await expect
    .poll(() =>
      bubbleHandle!.evaluate((node) => node.textContent?.trim() ?? ""),
    )
    .toBe(text);
  await expect(page.getByText(text, { exact: true })).toHaveCount(1);
});

test("keeps the queued-send explanation tied to admission rather than the current reply state", async ({
  page,
}) => {
  await page.goto("/");
  const queuedRow = page.getByTestId("message-queued");
  await expect(
    queuedRow.getByText("等当前回复结束后发送", { exact: true }),
  ).toBeVisible();
...
test("renders a deferred text-and-two-image send immediately and replaces it atomically", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => window.__companionFixture?.deferSend());
  ...
  await expect(
    page.locator('[data-testid^="message-submission:"]'),
  ).toHaveCount(1);
  await expect(page.locator('[data-testid^="image-submission:"]')).toHaveCount(
    2,
  );
  await expect(
    page.locator('[data-testid^="image-submission:"] .companion-media img'),
  ).toHaveCount(2);
  const submission = page
    .locator('[data-testid^="message-submission:"]')
    .filter({ hasText: "两张照片" });
  ...
  await expect
    .poll(() =>
      submission
        .locator(".companion-message-stack")
        .evaluate((node) =>
          [...node.children].map((child) =>
            child.matches(".companion-image-group")
              ? "images"
              : child.matches(".companion-bubble")
                ? "text"
                : "other",
          ),
        ),
  ...
  await page.evaluate(() => window.__companionFixture?.confirmSend());
  await expect(
    page.locator('[data-testid^="message-"]').filter({ hasText: "两张照片" }),
  ).toHaveCount(1);
  await expect
    .poll(() => submissionHandle!.evaluate((node) => node.isConnected))
    .toBe(true);
  await expect(submission.locator(".companion-image-entry")).toHaveCount(2);
...
test("keeps a submission lightbox preview alive until the dialog closes", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => window.__companionFixture?.deferSend());
  ...
  await page.getByRole("button", { name: "发送消息" }).click();
  const submissionImage = page
    .locator('[data-testid^="image-submission:"]')
    .first();
  await expect(submissionImage).toBeVisible();
  await submissionImage.getByRole("button", { name: /查看大图/ }).click();
  await expect(page.getByRole("button", { name: "关闭大图" })).toBeVisible();
  const revokedBeforeConfirmation = await page.evaluate(
    () => window.__companionFixture?.revoked() ?? 0,
  );
  ...
  await expect
    .poll(() => page.evaluate(() => window.__companionFixture?.revoked() ?? 0))
    .toBe(revokedBeforeConfirmation);

  await page.getByRole("button", { name: "关闭大图" }).click();
  await expect
    .poll(() => page.evaluate(() => window.__companionFixture?.revoked() ?? 0))
    .toBeGreaterThan(revokedBeforeConfirmation);
});

test("restores an identified rejection through the Session retirement callback", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => window.__companionFixture?.deferSend());
  ...
  await expect(
    page
      .locator(".companion-timeline .companion-recovery")
      .filter({ hasText: "这条消息没发出去，可以再试一次。" }),
  ).toHaveCount(1);
...
test("keeps an existing internal send error visible when a submission begins", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() =>
    window.__companionFixture?.seedInternalPromptError(),
  );
  await expect(
    page
      .locator(".companion-timeline .companion-recovery")
...
test("restores identified failed submissions and clears old-session sends", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => window.__companionFixture?.deferSend());
  ...
  await page.evaluate(() =>
    window.__companionFixture?.switchSession("weekend-plan"),
  );
  await expect(
    page.locator('[data-testid^="message-submission:"]'),
  ).toHaveCount(0);
...
test("keeps the capacity cue optional and makes its explanation keyboard reachable", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.evaluate(() =>
    window.__companionFixture?.setCapacity({
      pressureTokens: 8_000,
      projectedTokens: 18_432,
...
test("surfaces compaction lifecycle without exposing the private checkpoint", async ({
  page,
}) => {
  ...
  await page.evaluate(() =>
    window.__companionFixture?.setCapacity({
      projectedTokens: 20_000,
      contextWindow: 32_000,
    }),
  );
  await page.evaluate(() =>
    window.__companionFixture?.startCompaction("active"),
  );
  await expect(page.getByTestId("companion-continuity-status")).toHaveText(
    "正在整理记忆…",
  );
  await expect(page.locator(".companion-context-meter")).toHaveAttribute(
    "data-state",
    "active",
  );
  await page.evaluate(() =>
    window.__companionFixture?.finishCompaction("active"),
  );
  await expect(page.getByTestId("companion-continuity-status")).toHaveText(
    "整理记忆已完成",
  );
  await expect(page.getByTestId("continuity-record-active")).toHaveText(
    "已整理对话",
  );
  await expect(
    page.getByTestId("continuity-record-active"),
  ).not.toHaveAttribute("role", "status");
  await expect(page.getByTestId("continuity-record-active")).toHaveAttribute(
    "aria-live",
    "off",
  );
  await expect(page.getByTestId("companion-continuity-status")).toHaveAttribute(
    "role",
    "status",
  );
  ...
  await page.evaluate(() =>
    window.__companionFixture?.startCompaction("failed"),
  );
  await page.evaluate(() =>
    window.__companionFixture?.failCompaction("failed"),
  );
  await expect(page.getByTestId("companion-continuity-status")).toHaveText(
    "本次整理未完成，仍可继续对话",
  );
...
test("keeps active continuity still and semantic across both authored themes", async ({
  page,
}) => {
  ...
  await page.evaluate(() =>
    window.__companionFixture?.startCompaction("theme"),
  );
  const meter = page.locator(".companion-context-meter");
  await expect(meter).toHaveAttribute("data-state", "active");
  await expect
    .poll(() => meter.evaluate((node) => getComputedStyle(node).animationName))
    .toBe("none");
  const lightStroke = await meter
    .locator(".companion-context-meter-value")
...
test("uses image progress instead of a duplicate typing indicator, then resumes typing", async ({
  page,
}) => {
  await page.goto("/");
...
test("warms up a long typing wait with rotating non-repeating companion copy", async ({
  page,
}) => {
  await page.clock.install();
  await page.goto("/");
...
test("uses the composer action to stop a running reply without hiding queued messages", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__companionFixture?.stopCalls() ?? 0),
    )
    .toBe(1);
...
test("Svelte 5 bridge applies live identity and theme changes without remounting", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.getByTestId("companion-root")).toHaveAttribute(
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__companionFixture?.rootIsStable() ?? false),
    )
    .toBe(true);
  await page.evaluate(() =>
    window.__companionFixture?.setIdentity({
      companionName: "新灯",
      moodLabel: "愉快",
    }),
  );
  ...
  const statusClasses = {
    ready: "cmp-status-success",
    working: "cmp-status-warning",
    reconnecting: "cmp-status-error",
  } as const;
  for (const [status, className] of Object.entries(statusClasses)) {
    await page.evaluate(
      (next) =>
        window.__companionFixture?.setStatus(
          next as "ready" | "working" | "reconnecting",
        ),
      status,
    );
    ...
    await expect
      .poll(() =>
        presenceStatus.evaluate(
          (node) => getComputedStyle(node).backgroundColor,
        ),
      )
      .not.toBe("rgba(0, 0, 0, 0)");
    await expect(page.locator(".companion-presence")).toContainText(
      status === "ready"
        ? "在线"
        : status === "working"
          ? "正在输入…"
  ...
  const lightStatus = await presenceStatus.evaluate(
    (node) => getComputedStyle(node).backgroundColor,
  );
  await page.evaluate(() => window.__companionFixture?.setTheme("dark"));
  await expect(page.getByTestId("companion-root")).toHaveAttribute(
  ...
  await expect
    .poll(() =>
      presenceStatus.evaluate((node) => getComputedStyle(node).backgroundColor),
    )
    .not.toBe(lightStatus);
  await expect
    .poll(() =>
      page.evaluate(() => window.__companionFixture?.rootIsStable() ?? false),
    )
    .toBe(true);
  await page.evaluate(() => {
    window.__companionFixture?.dispose();
    window.__companionFixture?.dispose();
  });
  await expect.poll(() => page.getByTestId("companion-root").count()).toBe(0);
  expect(
    await page.evaluate(() => window.__companionFixture?.unmountCalls() ?? 0),
  ).toBe(1);
});

test("drawer uses one checkbox state across desktop and Pixel-sized layouts", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  if (testInfo.project.name === "pixel-7a") {
    ...
    await expect
      .poll(() =>
        page
          .locator(".companion-sidebar-overlay")
          .evaluate((node) => getComputedStyle(node).backgroundColor),
      )
      .toBe("rgba(0, 0, 0, 0)");
    await page
      .locator(".companion-sidebar-overlay")
      .click({ position: { x: 390, y: 120 } });
    await expect(toggle).not.toBeChecked();
    await expect
      .poll(() =>
        page.evaluate(() =>
          window.localStorage.getItem("dsh-companion:desktop-sidebar-open"),
        ),
      )
      .toBeNull();
    await page.reload();
...
test("chat shell has rendered Markdown, viewport scrolling, sessions, rounded focus, and an anchored relationship card", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop",
    "desktop geometry is the tight structural baseline",
  );
  await page.goto("/?theme=dark");
  await page.evaluate(() => document.fonts.ready);
  ...
  await expect(
    page.getByRole("button", { name: "收起对话列表" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "切换到对话：今晚的小星光" }),
  ).toHaveAttribute("aria-current", "true");
  await expect(
    page.getByRole("button", { name: "切换到对话：周末想去哪里" }),
  ).toBeVisible();
  ...
  const geometry = await page.evaluate(() => {
    ...
    const composeRow = document.querySelector<HTMLElement>(
      ".companion-compose-row",
    )!;
    ...
    return {
      ...
      composerBottomInset:
        window.innerHeight - composeRow.getBoundingClientRect().bottom,
  ...
  expect(
    Math.abs(geometry.rootHeight - geometry.viewportHeight),
  ).toBeLessThanOrEqual(1);
  expect(geometry.documentHeight).toBeLessThanOrEqual(
    geometry.viewportHeight + 1,
  );
  expect(geometry.appRect).toEqual({
    top: 0,
    left: 0,
  ...
  await page.locator(".companion-timeline").evaluate((node) => {
    node.scrollTop = 120;
  });
  await expect
    .poll(() =>
      page.locator(".companion-timeline").evaluate((node) => node.scrollTop),
    )
    .toBeGreaterThan(0);
  ...
  const focusStyle = await textarea.evaluate((node) => {
    ...
    const shellStyle = getComputedStyle(
      node.closest(".companion-compose-row")!,
    );
    return {
  ...
  expect(
    Math.abs(bubbles.incoming.avatarTop - bubbles.incoming.bubbleTop),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.abs(bubbles.outgoing.avatarTop - bubbles.outgoing.bubbleTop),
  ).toBeLessThanOrEqual(1);
...
test("Markdown renders GFM safely with aligned list levels", async ({
  ...
  await expect(
    gfm.getByRole("heading", { name: "今天的小清单" }),
  ).toBeVisible();
  ...
  await expect(gfm.locator("ul ul")).toHaveCSS(
    "list-style-position",
    "outside",
  );
...
test("initial chat presentation is already at the bottom with circular avatars and a stable profile popover", async ({
  page,
}) => {
  await page.addInitScript(() => {
    const state = window as unknown as {
      __companionScrollSamples: Array<{
    ...
    const sample = () => {
      ...
      if (timeline)
        state.__companionScrollSamples.push({
  ...
  const scrollSamples = await page.evaluate(
    () =>
      ...
      ).__companionScrollSamples,
  );
  const visibleSamples = scrollSamples.filter((sample) => sample.visible);
  expect(visibleSamples.length).toBeGreaterThan(0);
  expect(Math.abs(visibleSamples[0]!.distanceFromBottom)).toBeLessThanOrEqual(
    1,
  );
  expect(
    Math.abs(visibleSamples.at(-1)!.distanceFromBottom),
  ).toBeLessThanOrEqual(1);
...
test("captures readable Sticker Messenger and Night Voyage references", async ({
  page,
}, testInfo) => {
  ...
  for (const theme of ["light", "dark"] as const) {
    ...
    await page.screenshot({
      path: `fixture/screenshots/${theme === "light" ? "sticker-messenger" : "night-voyage"}-${device}.png`,
      fullPage: true,
    });
  }
});
