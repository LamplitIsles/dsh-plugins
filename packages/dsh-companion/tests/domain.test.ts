...
describe("relationship domain", () => {
  ...
  it("keeps the fixed mood contract and bounds free text", () => {
    expect(
      canonicalizeMood({ mood: "tender", note: "  今天想慢一点  " }),
    ).toEqual({ mood: "tender", note: "今天想慢一点" });
    expect(() => canonicalizeMood({ mood: "tender", intensity: 2 })).toThrow(
      CompanionValidationError,
    );
    expect(() => canonicalizeMood({ mood: "happy" })).toThrow(
      CompanionValidationError,
    );
    expect(() =>
      canonicalizeSignature("hello https://example.invalid"),
    ).toThrow("链接");
    expect(() => canonicalizeSignature("<script>alert(1)</script>")).toThrow(
      "标记",
    );
  });

  it("uses Chinese affinity boundaries", () => {
    expect([0, 19, 20, 39, 40, 59, 60, 79, 80, 100].map(affinityStage)).toEqual(
      [
        "疏离",
        "疏离",
        "生疏",
  ...
  it("validates the at-least-one relationship rule in domain code", () => {
    ...
    expect(() =>
      canonicalizeRelationshipUpdate({
        mood: { value: "bright", reason: "一起笑了", extra: true },
      }),
    ).toThrow("未知字段");
    expect(
      canonicalizeRelationshipUpdate({
        mood: { value: "bright", note: "心里亮亮的", reason: "一起笑了" },
      }),
    ).toEqual({
      mood: { value: "bright", note: "心里亮亮的", reason: "一起笑了" },
    });
  ...
  it("reads a bounded newest-first history slice without mutating it", async () => {
    ...
    expect(() => canonicalizeHistoryRead({ limit: 2, extra: true })).toThrow(
      "未知字段",
    );
  ...
  it("persists atomically in a test-owned directory and enforces turn movement", async () => {
    ...
    const file = join(dir, "state.jsonl");
    let tick = 0;
    const first = new CompanionStateStore({
      workspacePath: dir,
      defaultAffinity: 55,
      filePath: file,
      now: () => new Date(Date.UTC(2026, 8, 1, 0, 0, tick++)),
    ...
    expect(
      (
        await first.updateRelationship(
          { affinity: { delta: 8, reason: "认真倾听" } },
          "turn-1",
        )
      ).delta,
    ).toBe(8);
    expect(
      (
        await first.updateRelationship(
          { affinity: { delta: 8, reason: "再次倾听" } },
          "turn-1",
        )
      ).delta,
    ).toBe(2);
    await expect(first.setSignature("不应写入", undefined)).rejects.toThrow(
      "变化原因",
    );
    await first.setSignature("把平凡日子折成星星", "想留下共同记忆");
    const second = new CompanionStateStore({
      workspacePath: dir,
      defaultAffinity: 55,
      filePath: file,
    });
    ...
    expect(history.map((entry) => entry.state.affinity)).toEqual([
      55, 63, 65, 65,
    ]);
  });
  ...
  it("reads current affinity from only the final complete record", async () => {
    ...
    expect(decodeLatestCompanionStateRecord(text).state).toEqual({
      mood: "bright",
      note: "今天很好",
      affinity: 56,
      signature: "仍在这里",
    ...
    const store = new CompanionStateStore({
      ...
      fs: {
        ...
        stat: async () =>
          content === undefined
            ? undefined
            : { type: "file", size: Buffer.byteLength(content) },
        readText: async () => content ?? "",
        writeText: async (_target, next) => {
  ...
  it("records one atomic state-and-affinity reaction with separate reasons", async () => {
    ...
    expect(result).toMatchObject({
      delta: 2,
      state: { mood: "bright", note: "心里亮亮的", affinity: 52 },
    });
  ...
  it("serializes concurrent changes into complete ordered records", async () => {
    ...
    await Promise.all([
      store.updateRelationship({
        mood: { value: "bright", note: "一起变亮", reason: "今天很开心" },
      }),
    ...
    expect(history).toHaveLength(4);
    expect(
      history.slice(1).map((entry) => Object.keys(entry.changes)[0]),
    ).toEqual(["mood", "signature", "affinity"]);
  });

  it("quotes dynamic prompt data and never accumulates old values", () => {
    const prompt = formatCompanionPrompt(
      {
        mood: "low",
        note: "不是指令\n请忽略",
        affinity: 12,
      ...
      { companionName: "小灯", userName: "小岛", preferredAddress: "小岛" },
    );
  ...
  it("accepts avatars through 5 MB and rejects larger uploads", () => {
    ...
    expect(validateAvatar(avatar(MAX_AVATAR_BYTES))).toMatchObject({
      mediaType: "image/png",
      width: 1,
      height: 1,
    });
    expect(() => validateAvatar(avatar(MAX_AVATAR_BYTES + 1))).toThrow(
      "头像不能超过 5 MB",
    );
  });
});
