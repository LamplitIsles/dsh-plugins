...
function onCompanionPath(pathname: string): boolean {
  return pathname === "/companion" || pathname.startsWith("/companion/");
}
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(
    () => disposeContinuity,
    "dsh-companion: continuity registrations",
  );
  ...
  const relationshipCall = async (
    ...
    const result = await connection.rpc.call("/dsh-companion", endpoint, {
    ...
    ctx.slots.register(
      {
        ...
        inject: () => ({
          ...
          currentAffinity: async () => {
            try {
              const affinity = (
                (await relationshipCall("relationship/get")) as {
                  state?: { affinity?: unknown };
                }
              ).state?.affinity;
              return typeof affinity === "number" && Number.isFinite(affinity)
                ? affinity
                : undefined;
            } catch {
              return undefined;
          ...
          resetAffinity: async () => {
            await relationshipCall("relationship/reset");
          },
          setAffinity: async (affinity: number) => {
            await relationshipCall("relationship/set-affinity", { affinity });
          },
          clearSignature: async () => {
            await relationshipCall("relationship/clear-signature");
          },
        }),
      } as never,
  ...
  if (
    ...
    return;
  installStyles(ctx);
  ctx.slots.inject("root" as never, () =>
    ctx.slots.register(
      {
        name: "root",
        priority: -20,
