...
export function validateAvatar(value: unknown): AvatarInput {
  ...
  if (
    comma < 0 ||
    !/^data:[^;,]+;base64,/iu.test(record.data.slice(0, comma + 1))
  ) {
...
export function validateIdentitySettings(
  value: unknown,
  ...
  if (record.companionAvatar !== undefined)
    settings.companionAvatar = validateAvatar(record.companionAvatar);
  if (record.userAvatar !== undefined)
    settings.userAvatar = validateAvatar(record.userAvatar);
  return settings;
}

/** Decode the exact persisted wire record and reject unknown/oversized data. */
export function decodeCompanionState(
  value: unknown,
  defaultAffinity = 50,
): CompanionState {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
  ...
  const affinity =
    record.affinity === undefined
      ? clampAffinity(defaultAffinity)
      : record.affinity;
  if (
    typeof affinity !== "number" ||
    !Number.isSafeInteger(affinity) ||
    affinity < 0 ||
    affinity > 100
  ) {
...
export function decodeCompanionStateRecord(
  value: unknown,
  defaultAffinity = 50,
): CompanionStateRecord {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    throw new CompanionValidationError("状态记录不是对象。");
  const record = value as Record<string, unknown>;
  if (
    Object.keys(record).some((key) => !["at", "changes", "state"].includes(key))
  )
    throw new CompanionValidationError("状态记录包含未知字段。");
  if (
  ...
  if (
    ...
    throw new CompanionValidationError("状态记录变更无效。");
  const rawChanges = record.changes as Record<string, unknown>;
  const changeKeys = Object.keys(rawChanges);
  if (
    changeKeys.length === 0 ||
    changeKeys.some(
      (key) => !["seed", "mood", "affinity", "signature"].includes(key),
    )
  )
    throw new CompanionValidationError("状态记录变更无效。");
  if (
    rawChanges.seed !== undefined &&
    (rawChanges.seed !== true || changeKeys.length !== 1)
  )
    throw new CompanionValidationError("初始状态记录变更无效。");
  const state = decodeCompanionState(record.state, defaultAffinity);
  const changes: CompanionStateChanges =
    rawChanges.seed === true ? { seed: true } : {};
  if (rawChanges.mood !== undefined) {
    if (
      typeof rawChanges.mood !== "object" ||
      rawChanges.mood === null ||
      Array.isArray(rawChanges.mood)
    )
      throw new CompanionValidationError("状态记录的此刻状态变化无效。");
    ...
    if (mood.mood !== state.mood || mood.note !== state.note)
      throw new CompanionValidationError(
        "状态记录的此刻状态与完整状态不一致。",
      );
    changes.mood = {
  ...
  if (rawChanges.affinity !== undefined) {
    if (
      typeof rawChanges.affinity !== "object" ||
      rawChanges.affinity === null ||
      Array.isArray(rawChanges.affinity)
    )
      throw new CompanionValidationError("状态记录的亲近度变化无效。");
    const rawAffinity = rawChanges.affinity as Record<string, unknown>;
    if (
      Object.keys(rawAffinity).some(
        (key) => !["delta", "value", "reason"].includes(key),
      )
    )
      throw new CompanionValidationError("状态记录的亲近度变化包含未知字段。");
    if (
      !Number.isSafeInteger(rawAffinity.delta) ||
      (rawAffinity.delta as number) < -100 ||
      (rawAffinity.delta as number) > 100
    )
      throw new CompanionValidationError("状态记录亲近度变化无效。");
    if (
      typeof rawAffinity.value !== "number" ||
      !Number.isSafeInteger(rawAffinity.value) ||
      rawAffinity.value !== state.affinity
    )
      throw new CompanionValidationError("状态记录的亲近度与完整状态不一致。");
    const reason = canonicalizeOptionalChangeReason(rawAffinity.reason);
    changes.affinity = {
      delta: rawAffinity.delta as number,
      value: rawAffinity.value,
      ...(reason === undefined ? {} : { reason }),
    };
  }
  if (rawChanges.signature !== undefined) {
    if (
      typeof rawChanges.signature !== "object" ||
      rawChanges.signature === null ||
      Array.isArray(rawChanges.signature)
    )
      throw new CompanionValidationError("状态记录的签名变化无效。");
    const rawSignature = rawChanges.signature as Record<string, unknown>;
    if (
      Object.keys(rawSignature).some(
        (key) => key !== "value" && key !== "reason",
      )
    )
      throw new CompanionValidationError("状态记录的签名变化包含未知字段。");
    const signature = canonicalizeSignature(rawSignature.value);
    if (signature !== state.signature)
      throw new CompanionValidationError("状态记录的签名与完整状态不一致。");
...
export function encodeCompanionStateRecord(
  ...
  if (new TextEncoder().encode(encoded).byteLength > MAX_STATE_RECORD_BYTES)
    throw new CompanionValidationError("状态记录过大。");
  return `${encoded}\n`;
}

export function decodeCompanionStateHistory(
  text: string,
  defaultAffinity = 50,
): CompanionStateRecord[] {
  ...
  if (lines.some((line) => !line))
    throw new CompanionValidationError("Companion 状态历史包含空记录。");
  return lines.map((line) => {
    if (new TextEncoder().encode(line).byteLength > MAX_STATE_RECORD_BYTES)
      throw new CompanionValidationError("状态记录过大。");
    try {
      return decodeCompanionStateRecord(JSON.parse(line), defaultAffinity);
    } catch (error) {
      if (error instanceof CompanionValidationError) throw error;
      throw new CompanionValidationError("Companion 状态历史包含无效 JSON。");
    }
  });
}

/** Read the authoritative current state without replaying or decoding history. */
export function decodeLatestCompanionStateRecord(
  text: string,
  defaultAffinity = 50,
): CompanionStateRecord {
  if (new TextEncoder().encode(text).byteLength > MAX_STATE_BYTES)
  ...
  if (!line)
    throw new CompanionValidationError("Companion 状态历史没有当前记录。");
  if (new TextEncoder().encode(line).byteLength > MAX_STATE_RECORD_BYTES)
    throw new CompanionValidationError("状态记录过大。");
  try {
    return decodeCompanionStateRecord(JSON.parse(line), defaultAffinity);
    ...
    if (error instanceof CompanionValidationError) throw error;
    throw new CompanionValidationError("Companion 当前状态包含无效 JSON。");
  }
}
...
export class CompanionStateStore {
  ...
  getSnapshot(): CompanionState {
    return {
      ...this.state,
      ...(this.state.note === undefined ? {} : { note: this.state.note }),
    };
  }
  ...
  async readHistory(
    limitInput?: unknown,
    signal?: AbortSignal,
  ): Promise<CompanionStateRecord[]> {
    const limit = canonicalizeHistoryLimit(limitInput);
    await this.load(signal);
    return decodeCompanionStateHistory(this.historyText, this.defaultAffinity)
      .slice(-limit)
      .reverse();
  }
  ...
  async waitForChange(
    since: number,
    signal?: AbortSignal,
  ): Promise<{ revision: number; state: CompanionState }> {
    await this.load(signal);
    if (this.revision > since)
      return { revision: this.revision, state: this.getSnapshot() };
  ...
  private async loadFromDisk(signal?: AbortSignal): Promise<void> {
    if (this.fs) {
      const target = await this.fs.resolve(STATE_FILE, {
      ...
      if (info === undefined) {
        ...
        this.historyText = encodeCompanionStateRecord({
        ...
        if (info.type !== "file")
          throw new CompanionValidationError("Companion 状态不是文件。");
        ...
        const latest = decodeLatestCompanionStateRecord(
          text,
          this.defaultAffinity,
        );
      ...
      try {
        ...
        await nodeFs.mkdir(
          this.filePath.slice(0, this.filePath.lastIndexOf("/")),
          { recursive: true },
        );
        this.historyText = encodeCompanionStateRecord({
          at: this.now().toISOString(),
  ...
  private async ensureParentDirectory(signal?: AbortSignal): Promise<void> {
    if (this.fs?.mkdir)
      await this.fs.mkdir(STATE_DIRECTORY, { cwd: this.workspacePath, signal });
  }
  ...
  private async updateWithChanges(
    mutator: (current: CompanionState) => CompanionState,
    changes: CompanionStateChanges | (() => CompanionStateChanges),
    signal?: AbortSignal,
    afterPersist?: () => void,
  ): Promise<CompanionState> {
    await this.load(signal);
    await this.enqueue(async () => {
      const next = decodeCompanionState(
        mutator(this.getSnapshot()),
        this.defaultAffinity,
      );
      await this.persist(
        next,
  ...
  async setSignature(
    ...
    return this.updateWithChanges(
      (current) => ({ ...current, signature }),
      { signature: { value: signature, reason } },
      signal,
    );
  }
  ...
  async setAffinity(
    ...
    return this.updateWithChanges(
      ...
      () => ({
        affinity: { delta, value, reason: "用户在设置中校正了亲近度" },
      }),
      signal,
      () => this.turnTotals.clear(),
    );
  ...
  beginTurn(turnId: string): void {
    ...
    if (this.turnTotals.size > 32)
      this.turnTotals.delete(this.turnTotals.keys().next().value as string);
  }
  ...
  async updateRelationship(
    ...
    const result = await this.updateWithChanges(
      (current) => {
        ...
        const nextMood =
          update.mood === undefined
            ...
            : {
                ...withoutOldNote,
                mood: update.mood.value,
                ...(update.mood.note === undefined
                  ? {}
                  : { note: update.mood.note }),
              };
        ...
        applied =
          update.affinity.delta < 0
            ? Math.max(update.affinity.delta, room)
            : Math.min(update.affinity.delta, room);
      ...
      () => ({
        ...(update.mood === undefined
          ...
          : {
              mood: {
                value: update.mood.value,
                ...(update.mood.note === undefined
                  ? {}
                  : { note: update.mood.note }),
                reason: update.mood.reason,
              },
            }),
        ...(update.affinity === undefined
          ? {}
          : {
              affinity: {
                delta: applied,
                value: affinity,
                reason: update.affinity.reason,
              },
            }),
      }),
      signal,
    ...
    return {
      state: result,
      ...(update.affinity === undefined ? {} : { delta: applied }),
    };
  }
}
 ...
 */
export function formatCompanionPrompt(
  state: CompanionState,
...
export function selectCompanionSession(
  ...
  const authoritativeIds = ownership.sessionIds
    ? new Set(ownership.sessionIds)
    : undefined;
