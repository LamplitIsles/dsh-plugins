...
export function relationshipControlsWritable(
  readOnly: boolean,
  saving: boolean,
): boolean {
...
export function decodeClientSettings(
  value: unknown,
  ...
  const text = (key: string, fallback: string): string =>
    typeof record[key] === "string" && (record[key] as string).trim()
      ? (record[key] as string).trim().slice(0, 80)
      : fallback;
  const result: ClientSettings = {
    ...
    defaultAffinity:
      ...
      Number.isInteger(record.defaultAffinity)
        ? Math.max(0, Math.min(100, record.defaultAffinity))
        : 50,
  };
...
function decodeAvatar(value: unknown): ClientAvatar | undefined {
  ...
  if (
    ...
    return undefined;
  if (!record.data.startsWith(`data:${record.mediaType};base64,`))
    return undefined;
  if (
  ...
  return {
    data: record.data,
    mediaType: record.mediaType as ClientAvatar["mediaType"],
    width: record.width,
...
export async function readAvatar(file: File): Promise<ClientAvatar> {
  ...
  return {
    data,
    mediaType: file.type as ClientAvatar["mediaType"],
    ...dimensions,
  };
}
...
export function settingsPayload(
  ...
  return {
    ...
    preferredAddress: settings.preferredAddress.trim(),
    defaultAffinity: settings.defaultAffinity,
    ...(settings.companionAvatar
      ? { companionAvatar: settings.companionAvatar }
      : {}),
...
export function mergeCleanSettingsDraft(
  ...
  ) as (keyof ClientSettings)[]) {
    if (sameSettingValue(draft[key], previous[key]))
      (merged as Record<keyof ClientSettings, unknown>)[key] = next[key];
  }
  return merged;
}

/** Return only values that differ from the last Host-accepted baseline. */
export function changedSettingsPayload(
  draft: ClientSettings,
  ...
  return Object.fromEntries(
    Object.entries(draftPayload).filter(
      ([key, value]) => !sameSettingValue(value, baselinePayload[key]),
    ),
  );
}

/** Compare a planned write with the latest Host-accepted field value. */
export function settingValueAccepted(
  ...
  return (
    settings !== undefined &&
    sameSettingValue(
      (settingsPayload(settings) as Record<string, unknown>)[key],
      value,
    )
  );
}
