...
export function validateTtsPayload(
  raw: unknown,
  origin?: string,
): { url: string; mediaType: "audio/mpeg"; bytes: number } {
  ...
  if (value.startsWith("/") && !value.startsWith("//")) {
    if (!value.startsWith(SPEECH_AUDIO_ROUTE_PREFIX))
      throw new Error("audio-invalid");
    return { url: value, mediaType: "audio/mpeg", bytes: payload.bytes };
  }
  const currentOrigin =
    origin ??
    (typeof location === "object" && location ? location.origin : undefined);
  ...
  try {
    ...
    if (
      ...
      !parsed.pathname.startsWith(SPEECH_AUDIO_ROUTE_PREFIX)
    )
      throw new Error("audio-invalid");
...
export class TtsPreparationCache {
  ...
  prepare(
    ...
    entry.promise = rpc
      .synthesize(normalized, sessionId, signal)
      .then((raw) => {
  ...
  get(sessionId: string, text: string): PreparedAudio | undefined {
    return this.entries.get(
      `${sessionId}:${digestText(normalizeTtsText(text))}`,
    )?.value;
  }
  ...
  dispose(revoke = true): void {
    if (revoke) {
      for (const entry of this.entries.values())
        if (entry.value?.url.startsWith("blob:"))
