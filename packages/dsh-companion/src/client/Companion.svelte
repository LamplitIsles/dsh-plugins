<script lang="ts">
  ...
  export let imageLimits:
    | import("@deepseek-ai/dsh-attachment").ImageAttachmentLimits
    | undefined;
  ...
  $: syncWaitingState(
    typingVisible,
    `${sessions.find((session) => session.selected)?.id ?? "none"}:${latestSettledReplyKey(projection)}`,
  );
  ...
  function latestSettledReplyKey(value: CompanionProjection): string {
    for (let index = value.items.length - 1; index >= 0; index -= 1) {
      ...
      if (
        ...
        return item.projectionKey ?? item.id;
    }
    return "empty";
  }
  ...
  function readDesktopSidebarPreference(): boolean {
    ...
    try {
      return (
        window.localStorage.getItem(DESKTOP_SIDEBAR_STORAGE_KEY) !== "false"
      );
    } catch {
      return true;
    }
...
>
  <div class="cmp-drawer companion-app">
    <input
      id="companion-session-drawer"
      class="cmp-drawer-toggle"
      type="checkbox"
