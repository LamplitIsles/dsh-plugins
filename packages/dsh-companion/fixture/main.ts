...
mountStyleSheet(
  fixtureStyleContext,
  fixtureApplicationStyles,
  APPLICATION_STYLESHEET_ID,
  "fixture: application stylesheet",
);
mountStyleSheet(
  fixtureStyleContext,
  settingsCardStyles,
  SETTINGS_STYLESHEET_ID,
  "fixture: settings stylesheet",
);
const svgDocument =
  "<svg xmlns='http://www.w3.org/2000/svg' width='640' height='420' viewBox='0 0 640 420'><rect width='640' height='420' rx='34' fill='#ffc857'/><circle cx='180' cy='190' r='86' fill='#f26d85'/><circle cx='460' cy='190' r='86' fill='#76c9bc'/></svg>";
...
const mood =
  moodViews[query.get("mood") as keyof typeof moodViews] ?? moodViews.tender;
...
const markdownEdgeItems: CompanionProjection["items"] =
  query.get("markdown") === "edge"
    ? [
        {
          id: "markdown-gfm",
          messageKey: "markdown-gfm",
          kind: "text",
          side: "incoming",
          text: `## 今天的小清单\n\n- [x] 写完信\n\n- 一层\n  - 二层\n\n| 时刻 | 心情 | 记录 | 提醒 | 天气 |\n| --- | --- | --- | --- | --- |\n| 此刻 | 安静 | ${wideTableCell} | 记得喝水 | 微风 |\n\n<img src=x onerror=alert(1)>`,
          time: now - 22 * 60 * 60 * 1000,
        },
      ]
    : query.get("markdown") === "ordered"
      ? [
          {
            id: "markdown-ordered",
            messageKey: "markdown-ordered",
            kind: "text",
            side: "incoming",
            text: orderedMarkdown,
            time: now - 22 * 60 * 60 * 1000,
          },
        ]
      : [];
const projection: CompanionProjection = {
  items: [
    ...
    {
      id: "history-1",
      messageKey: "history-1",
      kind: "text",
      side: "incoming",
      text: "今天也见到你真好。**窗外的风**有一点点甜。\n\n- 收好今天的小星光\n- 慢慢讲给你听",
      time: now - 25 * 60 * 60 * 1000,
    },
    {
      id: "history-2",
      messageKey: "history-2",
      kind: "text",
      side: "outgoing",
      text: "我刚刚忙完，想听你说说今天。",
    ...
    {
      id: "history-3",
      messageKey: "history-3",
      kind: "text",
      side: "incoming",
      text: "那我把今天收集到的小小星光，慢慢讲给你听。",
      time: now - 23 * 60 * 60 * 1000,
    },
    ...markdownEdgeItems,
    {
      id: "date:today",
      messageKey: "date:today",
      kind: "notice",
      side: "incoming",
      tone: "info",
      text: "今天 · 8月31日",
    ...
    },
    {
      ...
      kind: "text",
      side: "incoming",
      text: "我们可以把一整天的喧闹放在门外，只留下这一小段安静的时间。",
      time: now - 180000,
    },
    {
      id: "imagegen:demo:loading",
      messageKey: "imagegen:demo:loading",
      kind: "image",
      side: "incoming",
      state: "loading",
      alt: "正在准备的海边图片",
    ...
    {
      id: "imagegen:demo:img",
      messageKey: "imagegen:demo:img",
      kind: "image",
      side: "incoming",
      state: "ready",
      attachment: {
        attachmentId: "demo" as never,
        mediaType: "image/png",
        name: "今晚的海",
        bytes: 1200,
        width: 640,
    ...
    {
      id: "imagegen:demo:failed",
      messageKey: "imagegen:demo:failed",
      kind: "image",
      side: "incoming",
      state: "failed",
      alt: "没有生成的图片",
      error: "这张图片没有完成，可以稍后再试。",
      time: now - 135000,
    },
    {
      id: "voice:demo:1:abc",
      messageKey: "voice:demo:1:abc",
      kind: "voice",
      side: "incoming",
      text: "如果累了，就先把肩膀放松下来。",
      status: "preparing",
    ...
    {
      id: "voice:demo:failed",
      messageKey: "voice:demo:failed",
      kind: "voice",
      side: "incoming",
      text: "这段语音暂时失败，但文字稿还在。",
      status: "preparing",
      time: now - 105000,
    },
    {
      id: "notice:reconnect",
      messageKey: "notice:reconnect",
      kind: "notice",
      side: "incoming",
      tone: "info",
      text: "连接有一点不稳，正在重新连接……",
...
function replaceProjectionItems(
  current: CompanionBridgeProps,
...
const voiceTranscriptionGate =
  query.get("voice") === "1"
    ? new Promise<void>((resolve) => {
        releaseVoiceTranscription = resolve;
      })
    : undefined;
...
let lastSend:
  | { text: string; images: readonly CompanionImageDraft[]; echoId: string }
  | undefined;
...
const fixtureProps: CompanionBridgeProps = {
  ...
  identity: {
    ...
    preferredAddress: "小岛",
    signature:
      query.get("signature") === "empty"
        ? ""
        : "把平凡日子折成星星，等风来时再写一行很长很长的晚安",
    ...mood,
    affinity: 67,
    affinityStage: "亲近",
  },
  actions: {
    send: async (
      ...
      await new Promise<void>((resolve, reject) => {
        pendingSends.push({ text, images, echoId, onRetire, resolve, reject });
      });
    },
    stop: async () => {
      stopCalls += 1;
    ...
    selectSession: async (sessionId: string) => {
      switchFixtureSession(sessionId);
    },
    loadOlder: async () => undefined,
    attachmentUrl: async () => {
      if (failedAttachmentLoads > 0) {
        failedAttachmentLoads -= 1;
    ...
    prepareVoice: async (text: string) => {
      if (text.includes("失败")) throw new Error("fixture voice failure");
      return "/dsh-speech/audio/fixture.mp3";
    },
    transcribeVoice: async (_recording: VoiceRecording) => {
      if (query.get("voice") === "fail")
        throw new Error("语音转写暂时不可用，请稍后重试。");
      if (voiceTranscriptionGate) await voiceTranscriptionGate;
      return { text: "来自麦克风的测试消息", expression: "sad" };
    },
  },
...
function appendDurableSend(
  ...
  images.forEach((draft, index) => {
    additions.push({
      ...
      state: "ready",
      attachment: {
        attachmentId: `fixture-att-${sendSequence}-${index}` as never,
        mediaType: draft.file.type as never,
  ...
  propsStore.update((current) =>
    replaceProjectionItems(current, [
      ...current.projection!.items.filter(
        (item) => !item.id.startsWith(`${echoId}:`),
      ),
      ...additions,
    ]),
  );
...
function removeSubmissionEcho(id: string): void {
  propsStore.update((current) =>
    replaceProjectionItems(
      current,
      current.projection!.items.filter((item) => !item.id.startsWith(`${id}:`)),
    ),
  );
}

function setPromptError(
  text: string,
  code: string,
  op: "send" | "stop" = "send",
...
function confirmLastSend(): void {
  ...
  propsStore.update((current) => ({
    ...current,
    projection: { ...current.projection!, status: "ready" },
  }));
}
...
function switchFixtureSession(sessionId: string): void {
  propsStore.update((current) => ({
    ...
    projection: {
      ...
      promptErrorKey: undefined,
      promptErrorOp: undefined,
      promptErrorCode: undefined,
      lastAgentError: undefined,
    },
  }));
}

const component = bridgeMode
  ? undefined
  : mount(CompanionBridge, { target: root, props: { propsStore } });
const mountedCompanionRoot = bridgeMode
  ? undefined
  : document.getElementById("dsh-companion");
  ...
  interface Window {
    __companionFixture?: {
      ...
      setReadiness(next: {
        ...
        relationship?: CompanionReadiness;
        session?: CompanionReadiness;
      }): void;
...
function updateLifecycle(
  ...
  const lifecycles = [
    ...rows.filter((row) => row.compactionId !== state.compactionId),
    state,
  ].sort(
    (left, right) =>
  ...
  return {
    ...
    continuity: {
      ...current.continuity,
      lifecycle: { lifecycles, latest: lifecycles.at(-1) },
    },
  };
}
window.__companionFixture = {
  replaceImage() {
    propsStore.update((current) =>
      replaceProjectionItems(
        current,
        current.projection!.items.map((item) =>
          item.kind === "image" && item.id === "imagegen:demo:img"
            ? {
                ...item,
                attachment: {
                  ...item.attachment!,
                  attachmentId: "demo-replacement" as never,
                },
              }
            : item,
        ),
  ...
  removeImage() {
    propsStore.update((current) =>
      replaceProjectionItems(
        current,
        current.projection!.items.filter(
          (item) => item.id !== "imagegen:demo:img",
        ),
      ),
    );
  },
  setTheme(theme) {
    propsStore.update((current) => ({ ...current, scheme: theme }));
  },
  setReadiness(next) {
    propsStore.update((current) => ({
      ...current,
  ...
  resolveVoice() {
    ...
    releaseVoiceTranscription = undefined;
  },
  setStatus(status) {
    propsStore.update((current) => ({
      ...current,
      projection: { ...current.projection!, status },
    }));
  },
  setRunning(running) {
    propsStore.update((current) => ({
      ...current,
      projection: { ...current.projection!, running },
    }));
  },
  finishImageGeneration() {
    propsStore.update((current) =>
      replaceProjectionItems(
        current,
        current.projection!.items.filter(
          (item) => item.id !== "imagegen:demo:loading",
        ),
      ),
    );
  },
  ...
  resolveSend() {
    settlePendingSend("resolve");
  },
  rejectSend(message = "host-send-rejected") {
    settlePendingSend("reject", message);
  },
  ...
  setCapacity(value) {
    propsStore.update((current) => ({
      ...current,
      continuity: { ...current.continuity, contextPressure: value },
    }));
  },
  startCompaction(id) {
  ...
  finishCompaction(id) {
    ...
    propsStore.update((current) => {
      const existing = current.continuity?.lifecycle?.lifecycles.find(
        (row) => row.compactionId === compactionId,
      );
      const state: CompactionLifecycleState = {
        compactionId,
        status: "complete",
        startSeq: existing?.startSeq ?? ++lifecycleSeq,
      ...
      const record = {
        id: `continuity:${compactionId}`,
        messageKey: `continuity:${compactionId}`,
      ...
      return replaceProjectionItems(next, [
        ...next.projection!.items.filter((item) => item.id !== record.id),
        record,
      ]);
    });
  },
  failCompaction(id) {
    ...
    propsStore.update((current) => {
      ...
      return updateLifecycle(current, {
        compactionId,
        status: "failed",
        startSeq: existing?.startSeq ?? ++lifecycleSeq,
        startedAt: existing?.startedAt ?? endedAt,
  ...
  rootIsStable: () =>
    !bridgeMode &&
    document.getElementById("dsh-companion") === mountedCompanionRoot,
  disposeStyles() {
    for (const dispose of styleDisposers) dispose();
  },
  dispose() {
    if (!disposed) {
      disposed = true;
