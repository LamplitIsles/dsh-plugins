...
## Decision for DSH Speech

Use exactly this one form in the DSH Speech prompt and parser:
...
For DSH Speech, the configured voice already comes from plugin Settings, so it does
...
DSH Speech should therefore parse the complete completed assistant text and display
