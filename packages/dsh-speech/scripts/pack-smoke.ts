...
try {
  ...
  const packageDir = join(install, "node_modules", "@lamplitisles", "dsh-speech");
  const manifest = JSON.parse(readFileSync(join(packageDir, "package.json"), "utf8")) as {
    name: string;
    peerDependencies?: Record<string, string>;
    dsh?: { client?: { platform?: string; inject?: string[] } };
  };
  if (manifest.name !== "@lamplitisles/dsh-speech" || manifest.dsh?.client?.platform !== "web") {
  ...
  for (const required of ["dsh-speech", "@lamplitisles/dsh-speech", "connection", "credentials", "settings", "systemPrompt", "sessions", "webServer"]) {
  ...
  const rpc = await jsonRequest(runtime.baseUrl, "/dsh-speech/synthesize", {
    type: "client-request",
  ...
  const namespace = settingsEnvelope.result?.value?.namespaces?.find((candidate) => candidate.ns === "dsh-speech");
  if (
