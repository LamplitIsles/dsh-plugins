...
import { DSH_SPEECH_SERVICE, RPC_CHANNEL, RPC_ENDPOINT } from "../src/gateway.js";
...
describe("host plugin composition", () => {
  it("registers native settings, static prompt, the sole RPC channel, and Cordis failure logging", async () => {
    ...
    expect(sectionCalls[0]).toEqual(expect.objectContaining({ name: "dsh-speech:tagged-output", text: SPEECH_SYSTEM_PROMPT }));
    expect(handleCalls[0]).toEqual([RPC_CHANNEL, expect.any(Function)]);
    expect(provideCalls[0]).toEqual([DSH_SPEECH_SERVICE, expect.objectContaining({ synthesize: expect.any(Function) })]);
    expect(routeCalls[0]).toEqual(expect.objectContaining({ kind: "prefix", path: "/dsh-speech/audio", handler: expect.any(Function) }));
    ...
    expect(hostError).toHaveBeenCalledWith("[dsh-speech] synthesis failed", expect.objectContaining({ category: "unavailable" }));
  ...
  it("publishes the Host service for its lifetime while preserving the browser RPC payload", async () => {
    ...
    try {
      apply(ctx as never);
      expect(ctx.get(DSH_SPEECH_SERVICE)).toEqual(expect.objectContaining({ synthesize: expect.any(Function) }));
      const handler = handleCalls[0]?.[1] as (endpoint: string, payload: unknown, signal: AbortSignal) => Promise<unknown>;
      ...
      expect(browserResult).toEqual({
        ...
        value: {
          ...
          url: expect.stringMatching(/^\/dsh-speech\/audio\/[a-f0-9]{64}\.mp3\?sessionId=session-a$/),
          bytes: 3
        }
      });
