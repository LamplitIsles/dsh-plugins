...
describe("host plugin composition", () => {
  it("registers native settings, static prompt, the sole RPC channel, and Cordis failure logging", async () => {
    ...
    expect(sectionCalls[0]).toEqual(
      expect.objectContaining({
        name: "dsh-speech:tagged-output",
        text: SPEECH_SYSTEM_PROMPT,
      }),
    );
    expect(handleCalls[0]).toEqual([RPC_CHANNEL, expect.any(Function)]);
    expect(provideCalls[0]).toEqual([
      DSH_SPEECH_SERVICE,
      expect.objectContaining({ synthesize: expect.any(Function) }),
    ]);
    expect(routeCalls[0]).toEqual(
      expect.objectContaining({
        kind: "prefix",
        path: "/dsh-speech/audio",
        handler: expect.any(Function),
      }),
    );
    const handler = handleCalls[0]![1] as (
      endpoint: string,
      payload: unknown,
      signal: AbortSignal,
    ) => Promise<unknown>;
    ...
    await expect(
      handler("other", { text: "你好" }, new AbortController().signal),
    ...
    await expect(
      handler(
        RPC_ENDPOINT,
        { text: "你好", sessionId: "missing-session" },
        new AbortController().signal,
      ),
    ).resolves.toMatchObject({ ok: false, error: { message: "unavailable" } });
    expect(hostError).toHaveBeenCalledWith(
      "[dsh-speech] synthesis failed",
  ...
  it("publishes the Host service for its lifetime while preserving the browser RPC payload", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "dsh-speech-host-service-"));
    const handleCalls: unknown[][] = [];
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockImplementation(async (_url, init) => {
        const body = JSON.parse(requestBody(init)) as { model?: string };
        return new Response(
          JSON.stringify(
            body.model === QWEN_ASR_MODEL
              ? {
                  output: {
                    choices: [
                      {
    ...
    ctx.provide("sessions", {
      get: (sessionId: string) =>
        sessionId === "session-a" ? { header: { cwd } } : undefined,
    });
    ctx.provide("webServer", { register: () => () => undefined });

    try {
      apply(ctx as never);
      expect(ctx.get(DSH_SPEECH_SERVICE)).toEqual(
        expect.objectContaining({ synthesize: expect.any(Function) }),
      );
      const handler = handleCalls[0]?.[1] as (
        endpoint: string,
      ...
      const browserResult = await handler(
        RPC_ENDPOINT,
      ...
      expect(browserResult).toEqual({
        ...
        value: {
          ...
          url: expect.stringMatching(
            /^\/dsh-speech\/audio\/[a-f0-9]{64}\.mp3\?sessionId=session-a$/,
          ),
          bytes: 3,
        },
      });
      expect(
        Object.keys(
          (browserResult as { value: Record<string, unknown> }).value,
        ),
      ).toEqual(["mediaType", "url", "bytes"]);
      const service = ctx.get(DSH_SPEECH_SERVICE)!;
      await expect(
        service.transcribe({
          sessionId: "session-a",
          mediaType: "audio/mpeg",
      ...
      expect(ctx.get(DSH_SPEECH_SERVICE)).toBeUndefined();
    } finally {
