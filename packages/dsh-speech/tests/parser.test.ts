...
describe("tagged assistant text", () => {
  it("keeps ordinary prose unchanged", () => {
    expect(parseTaggedText("你好，世界。")).toEqual({
      segments: [{ kind: "text", text: "你好，世界。" }],
    });
  });
  ...
  it("handles a tag split across streaming snapshots by parsing each complete value", () => {
    ...
    expect(
      parseTaggedText("[[tts:text]]半句完成[[/tts:text]]").passage?.text,
    ).toBe("半句完成");
  });
  ...
  it("leaves fenced examples and malformed tags visible", () => {
    ...
    expect(parseTaggedText("[[tts:text]]未闭合").segments[0]).toEqual({
      kind: "text",
      text: "[[tts:text]]未闭合",
    });
  });
  ...
  it("recognizes only the first valid pair; later tag syntax stays text", () => {
    const parsed = parseTaggedText(
      "[[tts:text]]第一句[[/tts:text]] [[tts:text]]第二句[[/tts:text]]",
    );
    expect(parsed.passage?.text).toBe("第一句");
    expect(parsed.segments.at(-1)).toEqual({
      kind: "text",
      text: " [[tts:text]]第二句[[/tts:text]]",
    });
  });

  it("rejects an empty or 241-code-point passage", () => {
    expect(
      parseTaggedText("[[tts:text]][[/tts:text]]").passage,
    ).toBeUndefined();
    expect(
      parseTaggedText(`[[tts:text]]${"你".repeat(241)}[[/tts:text]]`).passage,
    ).toBeUndefined();
  });
  ...
  it("does not treat nested tag syntax as spoken content", () => {
    ...
    expect(parseTaggedText(input).passage).toBeUndefined();
    expect(parseTaggedText(input).segments[0]).toEqual({
      kind: "text",
      text: input,
    });
  });
