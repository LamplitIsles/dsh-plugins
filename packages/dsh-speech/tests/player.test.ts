...
import {
  SpeechAudioPlayer,
  SpeechPlayer,
  clearSpeechPreparationCache,
} from "../src/player.js";
import type { BrowserAudioPayload } from "../src/gateway.js";

const payload: BrowserAudioPayload = {
...
describe("tagged Speech player", () => {
  it("prefetches audio when the finalized message mounts and renders a native player", async () => {
    ...
    const client = {
      synthesize: async (_text: string, sessionId: string) => {
        calls += 1;
  ...
  it("shares successful and in-flight preparation by session, voice, and normalized text", async () => {
    ...
    const first = one.prepare(
      "  你好\n",
      '["alibaba","qwen3-tts-flash","Maia"]',
      "session-a",
    );
    const second = two.prepare(
      "你好",
      '["alibaba","qwen3-tts-flash","Maia"]',
      "session-a",
    );
  ...
  it("keeps page preparation separate for different provider profiles", async () => {
    ...
    await remountedAlibaba.prepare(
      "同一句",
      '["alibaba","qwen3-tts-flash","Maia"]',
      "session-a",
    );
  ...
  it("keeps shared generation alive after disposal and lets a later mount reuse it", async () => {
    ...
    const client = {
      synthesize: async () => {
        calls += 1;
        return new Promise<BrowserAudioPayload>((resolve) => {
          settle = resolve;
        });
      },
    };
  ...
  it("keeps an already prepared player stable when the selected voice changes", async () => {
    ...
    await act(async () => {
      root!.update(
        createElement(SpeechAudioPlayer, {
          text: "同一句",
          profileKey: '["alibaba","qwen3-tts-flash","Other"]',
          sessionId: "session-a",
          client,
        }),
      );
  ...
  it("keeps the transcript visible when prefetching fails", async () => {
    ...
    expect(
      textChildren(
        player.findByProps({ "data-speech-transcript": true }).children,
      ),
    ).toContain("你好（旁白）");
    expect(
      textChildren(player.findByProps({ role: "status" }).children),
    ).toContain("Audio unavailable");
    root!.unmount();
  });
  ...
  it("shows the transcript and forgets a stale cache URL when audio loading fails", async () => {
    ...
    void act(() => root!.root.findByType("audio").props.onError());
    const failed = root!.root.findByProps({ "data-speech-state": "error" });
    expect(
      textChildren(
        failed.findByProps({ "data-speech-transcript": true }).children,
      ),
    ).toContain("缓存已丢失（旁白）");
    root!.unmount();
