...
function sessionStore(cwd: string, id = "session-a") {
  return {
    get: (candidate: string) =>
      candidate === id ? { header: { cwd } } : undefined,
  };
}
...
async function workspace(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "dsh-speech-gateway-"));
  temporaryDirectories.push(directory);
  return directory;
}

const credential = (ref: unknown) => ({
  value:
    ref === ALIBABA_CREDENTIAL_REF || ref === BYTEDANCE_CREDENTIAL_REF
      ? "secret"
      : "wrong",
  source: "test",
});
...
describe("provider-neutral Speech gateway", () => {
  it("normalizes independent defaults and keeps the profile key secret-free", () => {
    ...
    expect(profileFromSettings({ provider: "bytedance" })).toMatchObject({
      provider: "bytedance",
      voice: DEFAULT_BYTEDANCE_VOICE,
  ...
  it("accepts ByteDance SSE metadata frames with null audio data", async () => {
    ...
    await expect(
      gateway.synthesize({ sessionId: "session-a", text: "测试" }),
  ...
  it("classifies malformed, rejected, empty, and oversized provider results without exposing content", async () => {
    ...
    const rejected = new SpeechGateway({
      ...base,
      fetch: async () => jsonResponse({ message: "do not expose" }, 403),
    });
    await expect(
      rejected.handle(
        RPC_ENDPOINT,
        { sessionId: "session-a", text: "你好" },
    ...
    const malformed = new SpeechGateway({
      ...base,
      fetch: async () => sseResponse({ code: 0, data: "not base64!" }),
    });
    await expect(
      malformed.synthesize({ sessionId: "session-a", text: "坏" }),
    ).rejects.toMatchObject({ category: "provider-invalid-audio" });
    const business = new SpeechGateway({
      ...base,
      fetch: async () =>
        sseResponse({ code: 3001, message: "private provider detail" }),
    });
    await expect(
      business.synthesize({ sessionId: "session-a", text: "拒绝" }),
    ).rejects.toMatchObject({ category: "provider-rejected" });
    const empty = new SpeechGateway({
      ...base,
      fetch: async () => sseResponse({ code: 20000000 }),
    });
    await expect(
      empty.synthesize({ sessionId: "session-a", text: "空" }),
    ).rejects.toMatchObject({ category: "provider-invalid-audio" });
    const oversized = new SpeechGateway({
      ...base,
      fetch: async () =>
        sseResponse({
          code: 0,
          data: Buffer.alloc(MAX_AUDIO_BYTES + 1).toString("base64"),
        }),
    });
    await expect(
      oversized.synthesize({ sessionId: "session-a", text: "太大" }),
    ).rejects.toMatchObject({ category: "provider-invalid-audio" });
    expect(
      JSON.stringify(
        await rejected.handle(
          RPC_ENDPOINT,
          { sessionId: "session-a", text: "你好" },
          new AbortController().signal,
        ),
  ...
  it("reports safe ByteDance response and HTTP diagnostics without request text or credentials", async () => {
    ...
    const gateway = new SpeechGateway({
      sessions: sessionStore(cwd),
      credentials: {
        resolve: async () => ({ value: "provider-secret", source: "test" }),
      },
      getSettings: () => ({ provider: "bytedance", bytedanceVoice: "voice-a" }),
      fetch: async () =>
        sseResponse({
          code: 3001,
          message: "voice-a rejected provider-secret for 私密正文",
        }),
    ...
    const httpFailure = new SpeechGateway({
      ...
      fetch: async () =>
        jsonResponse(
          {
            code: "Forbidden",
            message: "provider-secret cannot synthesize 私密正文",
          },
          403,
        ),
      onFailure: (failure) => failures.push(failure),
  ...
  it("reports only a fixed issue category when a ByteDance body cannot be framed", async () => {
    ...
    const gateway = new SpeechGateway({
      ...
      fetch: async () =>
        new Response(
          new Uint8Array([
            0x1e, 0x7b, 0x22, 0x63, 0x6f, 0x64, 0x65, 0x22, 0x3a, 0x30, 0x7d,
          ]),
          {
  ...
  it("resolves only the selected credential and does not perform network I/O when it is absent", async () => {
    ...
    await expect(
      gateway.synthesize({ sessionId: "session-a", text: "没有密钥" }),
  ...
  it("keeps provider/voice cache identities distinct and reuses workspace artifacts", async () => {
    ...
    expect(
      cacheDigest("同句", { provider: "alibaba", alibabaVoice: "Maia" }),
    ).not.toBe(
      cacheDigest("同句", {
        provider: "bytedance",
        bytedanceVoice: DEFAULT_BYTEDANCE_VOICE,
    ...
    expect(
      ...
      cacheDigest("同句", { provider: "alibaba", alibabaVoice: "Other" }),
    );
    let calls = 0;
    const first = new SpeechGateway({
      sessions: sessionStore(cwd),
      credentials: {
        resolve: async () => ({ value: "secret", source: "test" }),
      },
    ...
    const expected = await first.synthesize({
      sessionId: "session-a",
      text: "缓存",
    });
    const second = new SpeechGateway({
      sessions: sessionStore(cwd),
      credentials: {
        resolve: async () => ({ value: "secret", source: "test" }),
      },
    ...
    await expect(
      second.synthesize({ sessionId: "session-a", text: "缓存" }),
    ).resolves.toEqual(expected);
    expect(calls).toBe(1);
    await expect(
      readFile(
        audioArtifactPath(cwd, cacheDigest("缓存", { provider: "alibaba" })),
      ),
    ).resolves.toEqual(Buffer.from([0x49, 0x44, 0x33]));
  });
    ...
    const gateway = new SpeechGateway({
      ...
      fetch: async () => {
        ...
        return provider === "bytedance"
          ? sseResponse({ code: 0, data: "SUQz" })
          : jsonResponse({ output: { audio: { data: "SUQz" } } });
      },
    });
  ...
  it("serves only a resolved workspace digest", async () => {
    ...
    const payload: BrowserAudioPayload = await gateway.synthesize({
      sessionId: "session-a",
      text: "路由",
    });
    const captured: {
  ...
  it("returns a bounded byte payload through the optional Host service and reuses the cache", async () => {
    ...
    const service = createDshSpeechService(gateway);
    ...
    expect(JSON.stringify(first)).not.toContain("dsh-speech/audio");
    expect(calls).toBe(1);
  });
  ...
  it("keeps Host service validation and cancellation typed", async () => {
    ...
    const service = createDshSpeechService(gateway);
    await expect(
      service.synthesize({ sessionId: "missing", text: "你好" }),
    ).rejects.toMatchObject({ category: "unavailable" });
    await expect(
      service.synthesize({ sessionId: "", text: "你好" }),
    ).rejects.toMatchObject({ category: "invalid-input" });
    await expect(
      service.synthesize({ text: "你好" } as never),
    ).rejects.toMatchObject({ category: "invalid-input" });
    ...
    await expect(
      service.synthesize({ sessionId: "session-a", text: "" }),
    ).rejects.toMatchObject({ category: "invalid-input" });
  });
  ...
  it("transcribes bounded audio with the fixed Qwen model and normalizes audio annotations", async () => {
    ...
    const refs: unknown[] = [];
    let request:
      | { body: any; headers: Headers; signal: AbortSignal | null | undefined }
      | undefined;
    const gateway = new SpeechGateway({
      sessions: sessionStore(cwd),
      credentials: {
        resolve: async (ref) => {
          refs.push(ref);
          return { value: "asr-secret", source: "test" };
        },
      },
      ...
      fetch: async (url, init) => {
        ...
        expect(requestUrl(url)).toBe(DASHSCOPE_ENDPOINT);
        return jsonResponse({
          output: {
            choices: [
              {
    ...
    expect(request?.body).toEqual({
      ...
      input: {
        messages: [
          {
            role: "user",
            content: [{ audio: "data:audio/ogg;base64,AAEC/w==" }],
          },
        ],
      },
  ...
  it("accepts the exact Base64 Data URL boundary and rejects the next byte before admission", async () => {
    ...
    const exactBytes =
      Math.floor((MAX_ASR_DATA_URL_BYTES - prefixLength) / 4) * 3;
    expect(prefixLength + Math.ceil(exactBytes / 3) * 4).toBe(
      MAX_ASR_DATA_URL_BYTES,
    );
    const refs: unknown[] = [];
    let requests = 0;
    ...
    const gateway = new SpeechGateway({
      ...
      fetch: async (_url, init) => {
        ...
        observedDataUrlLength =
          body.input.messages[0]!.content[0]!.audio.length;
        return jsonResponse({
          output: { choices: [{ message: { content: [{ text: "边界" }] } }] },
        });
      },
    });
    await expect(
      gateway.transcribe({
        sessionId: "session-a",
        mediaType,
        data: new Uint8Array(exactBytes),
      }),
    ...
    await expect(
      gateway.transcribe({
        sessionId: "session-a",
        mediaType,
        data: new Uint8Array(exactBytes + 1),
      }),
    ).rejects.toMatchObject({ category: "invalid-input" });
  ...
  it("rejects invalid audio and unavailable sessions before credential or provider admission", async () => {
    ...
    const gateway = new SpeechGateway({
      ...
      fetch: async () => {
        requests += 1;
        return jsonResponse({ output: { text: "unexpected" } });
      },
    });
    const valid = {
      sessionId: "session-a",
    ...
    await expect(
      gateway.transcribe({ ...valid, mediaType: "text/plain" }),
    ).rejects.toMatchObject({ category: "invalid-input" });
    await expect(
      gateway.transcribe({
        ...valid,
        data: new Uint8Array(MAX_ASR_DATA_URL_BYTES + 1),
      }),
    ).rejects.toMatchObject({ category: "invalid-input" });
    await expect(
      gateway.transcribe({ ...valid, data: [] }),
    ).rejects.toMatchObject({ category: "invalid-input" });
    await expect(
      gateway.transcribe({ ...valid, language: "xx" }),
    ).rejects.toMatchObject({ category: "invalid-input" });
    await expect(
      gateway.transcribe({ ...valid, sessionId: "missing" }),
    ).rejects.toMatchObject({ category: "unavailable" });
  ...
  it("classifies malformed and rejected ASR responses without exposing body fields", async () => {
    ...
    const malformed = new SpeechGateway({
      ...base,
      fetch: async () =>
        jsonResponse({ output: { text: 42, details: "private transcript" } }),
    });
    await expect(
    ...
    const rejected = new SpeechGateway({
      ...
      fetch: async () =>
        jsonResponse(
          { error: "provider-secret rejected private transcript" },
          429,
        ),
    });
    await expect(
  ...
  it("propagates cancellation to DashScope and classifies an aborted request", async () => {
    ...
    const pending = gateway.transcribe(
      {
        sessionId: "session-a",
        mediaType: "audio/mpeg",
        data: new Uint8Array([1]),
