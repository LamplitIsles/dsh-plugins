...
export interface DshSpeechSynthesisRequest {
  sessionId: string;
  text: string;
}

/** Bounded MP3 payload returned by the optional Host-facing `dshSpeech` service. */
export interface DshSpeechAudio {
  mediaType: "audio/mpeg";
  data: Uint8Array;
}

/** A short audio attachment accepted by the Host-only Qwen ASR operation. */
export interface DshSpeechTranscriptionRequest {
  sessionId: string;
  mediaType: string;
  data: Uint8Array;
...
export interface DshSpeechTranscription {
  text: string;
...
export const DSH_SPEECH_SERVICE = "dshSpeech" as const;

declare module "@deepseek-ai/cordis" {
  interface Context {
    /** Optional Host Speech capability supplied by DSH Speech when mounted. */
    dshSpeech: DshSpeechService;
  }
}
