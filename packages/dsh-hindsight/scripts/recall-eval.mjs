...
const VARIANTS = [
  ...
  {
    name: "observation_experience",
    types: ["observation", "experience"],
    dedupe: false,
  },
  { name: "baseline_dedup", source: "baseline", dedupe: true },
  {
    name: "observation_experience_dedup",
    source: "observation_experience",
    dedupe: true,
  },
  {
  ...
  {
    name: "baseline_semantic_035",
    source: "baseline",
    minSemantic: 0.35,
    rank: "semantic",
  },
  {
    name: "baseline_semantic_040",
    source: "baseline",
    minSemantic: 0.4,
    rank: "semantic",
  },
  {
    name: "observation_experience_semantic",
    source: "observation_experience",
    requireSemantic: true,
    rank: "semantic",
  },
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--details") options.details = true;
      ...
      options.maxTokens = positiveInteger(argv[++index], arg);
    else if (arg === "--top-k")
      options.topK = positiveInteger(argv[++index], arg);
...
function normalizeMarkers(value, field, caseId) {
  ...
  if (!Array.isArray(value))
    throw new Error(`${caseId}.${field} must be an array`);
  return value.map((marker, index) => {
    if (typeof marker === "string" && marker.trim())
      return { label: marker, terms: [marker] };
    if (
      !marker ||
      typeof marker !== "object" ||
      typeof marker.label !== "string" ||
      !Array.isArray(marker.terms)
    ) {
      throw new Error(
        `${caseId}.${field}[${index}] must be a string or { label, terms[] }`,
      );
    }
    const terms = marker.terms.filter(
      (term) => typeof term === "string" && term.trim(),
    );
    if (!terms.length)
      throw new Error(`${caseId}.${field}[${index}] has no terms`);
...
function validateCases(input) {
  ...
  if (!Array.isArray(cases) || !cases.length)
    throw new Error(
      "Query file must contain a non-empty array or { cases: [...] }",
    );
  const ids = new Set();
  return cases.map((entry, index) => {
    ...
    if (typeof entry.query !== "string" || !entry.query.trim())
      throw new Error(`${id}.query must be non-empty`);
    if (
      entry.queryTimestamp !== undefined &&
      Number.isNaN(Date.parse(entry.queryTimestamp))
    ) {
...
function markerHits(markers, results) {
  ...
  return markers.map((marker) => ({
    label: marker.label,
    hit: marker.terms.some((term) =>
      texts.some((text) => text.includes(normalizeText(term))),
    ),
  }));
}
...
function summarizeVariant(variant, rawResults, latencyMs, testCase, options) {
  ...
  if (variant.requireSemantic)
    candidates = candidates.filter(
      (result) => typeof result.scores?.semantic === "number",
    );
  if (variant.minSemantic !== undefined) {
    candidates = candidates.filter(
      (result) =>
        typeof result.scores?.semantic === "number" &&
        result.scores.semantic >= variant.minSemantic,
    );
  }
  if (variant.rank === "semantic") {
    candidates = candidates.toSorted(
      (left, right) =>
        (right.scores?.semantic ?? -Infinity) -
        (left.scores?.semantic ?? -Infinity),
    );
  }
  ...
  return {
    ...
    retainedChars: results.reduce(
      (sum, result) => sum + normalizeText(result.text).length,
      0,
    ),
    typeCounts,
...
function aggregate(cases) {
  ...
  for (const testCase of cases) {
    for (const variant of testCase.variants) {
      ...
      if (variant.recallExpected) {
        aggregateVariant.goldHits += variant.gold.filter(
          (item) => item.hit,
        ).length;
        aggregateVariant.goldTotal += variant.gold.length;
      }
      aggregateVariant.unwantedHits += variant.unwanted.filter(
        (item) => item.hit,
      ).length;
  ...
  return Object.fromEntries(
    Object.entries(variants).map(([name, value]) => {
      const sortedLatency = value.latencyMs.toSorted(
      ...
      return [
        name,
        {
          ...
          unwantedHitRate: value.unwantedTotal
            ? value.unwantedHits / value.unwantedTotal
            : null,
          meanResultsWhenRecallUnneeded: value.unneededQueries
            ? value.unneededRetained / value.unneededQueries
            : null,
          correctEmptyRateWhenRecallUnneeded: value.unneededQueries
            ? value.unneededEmpty / value.unneededQueries
            : null,
...
async function main() {
  ...
  for (const testCase of testCases) {
    ...
    for (const variant of VARIANTS.filter((entry) => !entry.source)) {
      recalled[variant.name] = await recall(
        client,
        options.bank,
        testCase,
        variant,
    ...
    JSON.stringify(
      {
        ...
        privacy: options.details
          ? "raw text included by explicit --details"
          : "raw query and memory text omitted",
        settings: {
          budget: options.budget,
          maxTokens: options.maxTokens,
          topK: options.topK,
