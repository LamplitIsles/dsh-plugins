...
describe("HindsightClient", () => {
  ...
  it("passes companion recall and retain settings through the official SDK", async () => {
    ...
    await client.retain(
      "session-1",
      2,
      [{ role: "user", content: "记住我喜欢短句" }],
      "replace",
    );
    await client.retain(
      "session-1",
      3,
      [{ role: "user", content: "也记住这一句" }],
      "append",
    );
    ...
    const retained = requests
      .slice(1)
      .map((request) => request.body.items as Array<Record<string, unknown>>);
    ...
    const identicalTurn = [
      { role: "user" as const, content: "这句内容在不同回合相同" },
    ];
    ...
    const operationIds = requests
      .slice(3)
      .map((request) => request.body.operation_id as string);
