...
async function configFile(config: unknown): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "dsh-hindsight-dsh-"));
...
describe("DSH hooks", () => {
  it("recalls every direct user turn, replaces a full session once, then appends completed turns", async () => {
    ...
    const fetch = vi.fn<typeof globalThis.fetch>(async (input, init) => {
      ...
      if (request.url.endsWith("/memories/recall")) {
        return hindsightJson({
          results: [{ id: "memory-1", text: "Neil likes concise Chinese." }],
        });
      }
    ...
    const events: unknown[] = [
      { type: "turn/start", data: { turn: 1 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "昨天聊的事情" }],
        },
      },
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "我记得。" }] } },
      },
      { type: "turn/start", data: { turn: 2 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "今天也想短一点。" }],
        },
      },
      {
        type: "user/message",
        data: {
          ...
          content: [{ type: "text", text: "do not retain me" }],
        },
      },
      {
        type: "assistant/message",
    ...
    expect(JSON.stringify(decision.messages?.[1])).toContain(
      "Current host time",
    );
    expect(JSON.stringify(decision.messages?.[1])).toContain(
      "Historical memories",
    ...
    events.push(
      { type: "turn/start", data: { turn: 3 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "这是后来的一句。" }],
        },
      },
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "我收到啦。" }] } },
      },
    );
  ...
  it("does not inject the Hindsight context when companion memory is disabled", async () => {
    ...
    const hooks = createDshHooks({ configPath });
    const agent = {
      session: { header: { id: "session-time" }, snapshotEvents: () => [] },
    };
  ...
  it("injects a memory returned on consecutive eligible direct turns", async () => {
    ...
    const fetch = vi.fn<typeof globalThis.fetch>(async () =>
      hindsightJson({
        results: [
          {
            id: "memory-1",
            text: "Neil likes concise Chinese.",
            type: "observation",
          },
        ],
      }),
    ...
    const events: unknown[] = [
      { type: "turn/start", data: { turn: 1 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "请记住我的回复偏好" }],
        },
      },
      { type: "turn/start", data: { turn: 2 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "还是按那个偏好回复" }],
        },
      },
    ];
    const agent = {
    ...
    const first = await hooks.preStep(
      { agent, turn: 1, signal: new AbortController().signal },
      async () => ({
        kind: "enter",
        messages: [
    ...
    const second = await hooks.preStep(
      { agent, turn: 2, signal: new AbortController().signal },
      async () => ({
        kind: "enter",
        messages: [
    ...
    expect(JSON.stringify(first.messages?.[1])).toContain(
      "Neil likes concise Chinese.",
    );
    expect(JSON.stringify(second.messages?.[1])).toContain(
      "Neil likes concise Chinese.",
    );
  ...
  it("waits only for the asynchronous retain acknowledgement before closing a turn", async () => {
    ...
    const fetch = vi.fn<typeof globalThis.fetch>(
      () =>
        new Promise<Response>((resolve) => {
          acknowledge = () =>
            resolve(hindsightJson({ operation_id: "queued" }));
        }),
    );
    globalThis.fetch = fetch;
    const hooks = createDshHooks({ configPath });
    const events: unknown[] = [
      { type: "turn/start", data: { turn: 1 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "请记住这件事" }],
        },
      },
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "我会记住。" }] } },
      },
    ];
    const agent = {
      session: {
    ...
    let settled = false;
    const stopping = hooks.turnStopping({ agent, turn: 1 }).then(() => {
      settled = true;
    });
  ...
  it("serializes session retain submissions so the first replace precedes later appends", async () => {
    ...
    const events: unknown[] = [
      ...
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "第一句回复" }] } },
      },
      { type: "turn/start", data: { turn: 2 } },
      {
        type: "user/message",
      ...
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "第二句回复" }] } },
      },
    ];
    const agent = {
      session: {
    ...
    expect(
      (
        requests[0]?.body.items as Array<Record<string, unknown>> | undefined
      )?.[0],
    ).toMatchObject({ update_mode: "replace" });

    acknowledgements.shift()?.(hindsightJson({ operation_id: "first" }));
    await vi.waitFor(() => expect(fetch).toHaveBeenCalledTimes(2));
    expect(
      (
        requests[1]?.body.items as Array<Record<string, unknown>> | undefined
      )?.[0],
    ).toMatchObject({ update_mode: "append" });
  ...
  it("repairs the complete session after an append acknowledgement fails", async () => {
    ...
    const events: unknown[] = [
      ...
      { type: "turn/start", data: { turn: 2 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "失败但必须保留的第二句" }],
        },
      },
      {
        type: "assistant/message",
      ...
      { type: "turn/start", data: { turn: 3 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "触发修复的第三句" }],
        },
      },
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "第三句回复" }] } },
      },
      { type: "turn/start", data: { turn: 4 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "修复后的第四句" }],
        },
      },
      {
        type: "assistant/message",
        data: { message: { content: [{ type: "text", text: "第四句回复" }] } },
      },
    ];
    const agent = {
      session: {
    ...
    const retained = requests.map(
      (request) => (request.body.items as Array<Record<string, unknown>>)[0],
    );
    expect(retained.map((item) => item?.update_mode)).toEqual([
      "replace",
      "append",
      "replace",
      "append",
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining("retain submission failed"),
    );
  });
  ...
  it("replaces the full session when its selected bank changes", async () => {
    ...
    const events: unknown[] = [
      { type: "turn/start", data: { turn: 1 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "旧 bank 的内容" }],
        },
      },
      { type: "turn/start", data: { turn: 2 } },
      {
        ...
        data: {
          source: { kind: "user" },
          content: [{ type: "text", text: "新 bank 也应有完整历史" }],
        },
      },
    ];
    const agent = {
  ...
  it("releases turn de-duplication state when a session is disposed", async () => {
    ...
    void hooks.turnStopping({ agent, turn: 1 });
    await vi.waitFor(() => expect(fetch).toHaveBeenCalledTimes(1));
  ...
  it("exposes only deliberate reflection with its deadline and cancellation", async () => {
    ...
    const on = vi.fn<(...args: unknown[]) => unknown>();
    ...
    >((_namespace: unknown, _schema: unknown, _options?: unknown) => ({
      get: () => ({ bankId: "yuki" }),
    }));
    ...
    expect(registerSettings.mock.calls[0]?.[0]).toBe("dsh-hindsight");
    ...
    const reflecting = reflect?.execute(
      { query: "summarize patterns" },
