...
export function defaultConfigPath(): string {
  return (
    process.env.HINDSIGHT_CONFIG ||
    resolve(homedir(), ".hindsight", "coding-agent.json")
  );
}
...
function recallSettings(
  ...layers: Array<Partial<RecallSettings> | undefined>
): RecallSettings {
  const combined = layers.reduce<Partial<RecallSettings>>(
    (result, layer) => ({ ...result, ...layer }),
    {},
  );
  const types = Array.isArray(combined.types)
    ? combined.types.filter(
        (type): type is RecallSettings["types"][number] =>
          type === "world" || type === "experience" || type === "observation",
      )
    : DEFAULT_RECALL.types;
  return {
    budget:
      combined.budget === "mid" || combined.budget === "high"
        ? combined.budget
        : "low",
    maxTokens: positiveInteger(
      combined.maxTokens,
      DEFAULT_RECALL.maxTokens,
      4_000,
    ),
    types: types.length ? types : DEFAULT_RECALL.types,
    preferObservations:
      combined.preferObservations ?? DEFAULT_RECALL.preferObservations,
    topK: positiveInteger(combined.topK, DEFAULT_RECALL.topK, 20),
    contextTurns: positiveInteger(
      combined.contextTurns,
      DEFAULT_RECALL.contextTurns,
      8,
    ),
    maxQueryChars: positiveInteger(
      combined.maxQueryChars,
      DEFAULT_RECALL.maxQueryChars,
      4_000,
    ),
    timeoutMs: positiveInteger(
      combined.timeoutMs,
      DEFAULT_RECALL.timeoutMs,
      20_000,
    ),
  };
}
...
export function resolveCompanionConfig(
  ...
  return {
    ...
    apiUrl,
    apiToken:
      typeof effective.apiToken === "string" && effective.apiToken
        ? effective.apiToken
        : undefined,
    bankId,
