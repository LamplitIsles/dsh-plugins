...
corepack pnpm --filter @lamplitisles/dsh-hindsight run build
dsh plugin --profile web add file:/absolute/path/to/dsh-plugins/packages/dsh-hindsight
```

The bundle mounts `dsh-hindsight`. It does not alter the official Codex
...
The output should include an enabled `dsh-hindsight` row. A running DSH host
must be restarted after changing its bundle list.

For a published release, install the public package directly:

```bash
dsh plugin --profile web add @lamplitisles/dsh-hindsight
```
...
host rather than through a proxied mobile connection.
...
Run the package checks from the workspace root. The tests use fake Hindsight
HTTP responses and test-owned temporary config directories, so they never read
or modify a live bank:

```bash
corepack pnpm install --frozen-lockfile
corepack pnpm --filter @lamplitisles/dsh-hindsight run typecheck
corepack pnpm --filter @lamplitisles/dsh-hindsight run test
corepack pnpm --filter @lamplitisles/dsh-hindsight run build
DSH_CLI=/absolute/path/to/dsh corepack pnpm --filter @lamplitisles/dsh-hindsight run pack-smoke
```

The packed smoke installs the artifact into a disposable DSH `0.1.2-rc.1` Web
profile, verifies Settings and the Web bootstrap, and loads the client through
the real Loader.
...
DSH_CLI=/absolute/path/to/dsh corepack pnpm run release:prepare -- \
  @lamplitisles/dsh-hindsight v0.1.0 .release-artifacts/dsh-hindsight
```
...
```bash
npm publish .release-artifacts/dsh-hindsight/lamplitisles-dsh-hindsight-0.1.0.tgz \
  --access public --tag latest
```

Versions remain independent across the workspace. Release preparation does not
publish, deploy, provision npm trust, or manage credentials; use `og` for
Forgejo operations.
