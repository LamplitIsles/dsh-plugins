...
describe("session selection", () => {
  it("uses authoritative workspace membership and deterministic activity tie breaks", () => {
    const workspace = {
      id: "w",
      sessionIds: ["blank", "archived", "child", "older", "newer", "tie-b"],
    };
    const inspections = new Map([
      ...
      [
        "child",
        { meta: { id: "child", origin: "subagent" }, events: [userEvent(999)] },
      ],
  ...
  it("returns no candidate when every member is blank, archived, or a subagent", () => {
    ...
    const inspections = new Map([
      ...
      [
        "child",
        { meta: { id: "child", origin: "subagent" }, events: [userEvent(1)] },
      ],
    ]);
    expect(
      selectMostRecentEligibleSession(workspace, inspections, new Set()),
    ).toBeUndefined();
  });
});
