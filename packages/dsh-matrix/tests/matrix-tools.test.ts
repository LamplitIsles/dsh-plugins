...
function definitions(client: MatrixClientLike, roomId = "!allowed:example") {
  return createMatrixToolDefinitions({
    getClient: () => client,
    roomId,
    isReady: () => true,
  });
...
describe("fixed-room Matrix tools", () => {
  it("lists only a bounded, deterministic current joined roster with display labels", async () => {
    ...
    const result = (await definitions(client)[0]!.execute({}, execution())) as {
      members: Array<{ userId: string; displayName: string }>;
    };
    expect(requestedRoom).toBe("!allowed:example");
    expect(result.members).toHaveLength(MAX_ROOM_MEMBERS);
    expect(result.members[0]).toEqual({
      userId: "@member-001:example",
      displayName: "Member 001",
    });
    expect(
      result.members.every(
        (member) =>
          member.userId.startsWith("@member-") &&
          member.displayName.startsWith("Member "),
      ),
    ).toBe(true);
    ...
    const client: MatrixClientLike = {
      getRoom: () => ({
        getJoinedMembers: () => [
          {
            userId: "@same:example",
            name: "Same (@same:example)",
            membership: "join",
          },
          { userId: "@same:example", name: "stale", membership: "join" },
          {
            userId: "@other:example",
            displayName: "Same (@other:example)",
            membership: "join",
          },
        ],
      }),
  ...
  it("bounds the rendered roster contribution", async () => {
    const client: MatrixClientLike = {
      getRoom: () => ({
        getJoinedMembers: () =>
          Array.from({ length: MAX_ROOM_MEMBERS }, (_, index) => ({
  ...
  it("sends one ordinary fixed-room m.text event with no relation", async () => {
    ...
    const tool = definitions(client, "!fixed:example").find(
      (candidate) => candidate.name === MATRIX_SEND_MESSAGE,
    )!;
    const result = await tool.execute(
      { body: "hello group", roomId: "!attacker:example" } as any,
      execution(),
    );
  ...
  it("reads recent server messages chronologically, including prior bot messages", async () => {
    ...
    const client: MatrixClientLike = {
      getRoom: () => ({
        getMember: (userId) => ({
          userId,
          name: userId === "@bot:example" ? "Companion" : "Alice",
        }),
      }),
          ...
          chunk: [
            {
              event: {
                ...
                event_id: "$bot",
                sender: "@bot:example",
                content: { msgtype: "m.text", body: "previous reply" },
              },
            },
            {
              event: {
            ...
            },
            {
              event: {
                ...
                event_id: "$human",
                sender: "@alice:example",
                content: { msgtype: "m.text", body: "latest question" },
              },
            },
          ],
        };
    ...
    const tool = definitions(client).find(
      (candidate) => candidate.name === MATRIX_READ_RECENT_MESSAGES,
    )!;
    await expect(tool.execute({ last: 10 }, execution())).resolves.toEqual({
      messages: [
        {
          eventId: "$human",
  ...
  it("paginates past filtered events and excludes formatted HTML", async () => {
          ...
          chunk: [
            {
              event: {
                ...
                event_id: "$older",
                sender: "@alice:example",
                content: { msgtype: "m.text", body: "usable" },
              },
            },
          ],
        };
    ...
    await expect(
      readRecentMatrixMessages(client, "!allowed:example", 1),
    ).resolves.toMatchObject([{ eventId: "$older", text: "usable" }]);
  ...
  it("allows replies to verified server-history events from any turn", async () => {
    ...
    const client: MatrixClientLike = {
      fetchRoomEvent: async (roomId, eventId) => {
        if (roomId === "!allowed:example" && eventId === "$history")
          return {
            getId: () => "$history",
      ...
      sendMessage: async (_roomId, content) => {
        sent.push(content);
      },
    };
    const tool = definitions(client).find(
    ...
    await expect(
      tool.execute({ body: "reply", replyToEventId: "$history" }, execution()),
    ).resolves.toEqual({ sent: true });
    await expect(
      tool.execute({ body: "nope", replyToEventId: "$unknown" }, execution()),
    ).rejects.toThrow("configured Matrix room history");
    expect(sent).toEqual([
      {
        msgtype: "m.text",
        body: "reply",
  ...
  it("resolves exact current display labels to intentional Matrix mentions", async () => {
    ...
    await tool.execute(
      { body: "hello @people", mentions: ["Alice", "Alice", "Bob"] },
      execution(),
    );
    expect(sent).toEqual([
      {
  ...
  it("rejects stale, direct-ID, ambiguous, and special labels before sending", async () => {
    ...
    for (const label of [
      ...
      await expect(
        tool.execute({ body: "must not send", mentions: [label] }, execution()),
      ).rejects.toThrow(
    ...
    await expect(
      tool.execute(
        { body: "must not send", mentions: ["Missing"] },
        execution(),
      ),
  ...
  it("bounds the complete adversarial correction error without stable IDs", async () => {
    ...
    try {
      await tool.execute(
        { body: "must not send", mentions: ["unknown"] },
        execution(),
      );
    } catch (error) {
      failure = error;
  ...
  it("fails closed when the local roster changes after label resolution", async () => {
    ...
    const tool = definitions(client)[2]!;
    await expect(
      tool.execute({ body: "stale", mentions: ["Alice"] }, execution()),
  ...
  it("rejects invalid bodies, unavailable connections, and cancellation with bounded errors", async () => {
    ...
    await expect(tool.execute({ body: "   " }, execution())).rejects.toThrow(
      "non-empty",
    );
    await expect(
      tool.execute(
        { body: "x".repeat(MAX_MATRIX_TOOL_BODY_CHARS + 1) },
        execution(),
      ),
    ).rejects.toThrow("at most");

    const unavailable = definitions(
      undefined as unknown as MatrixClientLike,
    )[2]!;
    await expect(
      unavailable.execute({ body: "hello" }, execution()),
    ).rejects.toThrow("unavailable");

    const controller = new AbortController();
    controller.abort();
    await expect(
      tool.execute({ body: "hello" }, execution(controller.signal)),
    ).rejects.toThrow("cancelled");
    const broken = definitions({
      getRoom: () => {
        throw new Error("secret token from Matrix");
      },
    } as MatrixClientLike)[0]!;
    await expect(broken.execute({}, execution())).rejects.toThrow(
      "unavailable",
    );
    await expect(broken.execute({}, execution())).rejects.not.toThrow(
      "secret token",
    );
  });
