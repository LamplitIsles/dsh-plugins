...
function apiFixture(
  options: { rejectCredential?: boolean; writable?: boolean } = {},
) {
...
function props(
  ...
  readiness: MatrixSettingsCardProps["readiness"] = {
    get: async () => ({
      ok: true,
      value: { state: "bound", workspaceId: "w1", sessionId: "s1" },
    }),
  },
...
async function renderCard(cardProps: MatrixSettingsCardProps) {
  ...
  await act(async () => {
    renderer = create(<MatrixSettingsCard {...cardProps} />);
  });
  const headers = renderer.root.findAllByProps({
    "data-plugin-card-header": "dsh-matrix",
  });
  if (headers[0])
    await act(async () => {
...
describe("MatrixSettingsCard", () => {
  it("matches native plugin disclosure behavior by starting collapsed", async () => {
    let renderer!: ReactTestRenderer;
    await act(async () => {
      renderer = create(<MatrixSettingsCard {...props(scopeFixture())} />);
    });
    ...
    expect(
      renderer.root.findAllByProps({ "data-settings-field": "homeserverUrl" }),
    ).toHaveLength(0);
    await act(async () => {
      header.props.onClick();
    });
    expect(
      renderer.root.findByProps({ "data-settings-field": "homeserverUrl" }),
    ).toBeTruthy();
    renderer.unmount();
  });
  ...
  it("hides an unavailable namespace and renders workspace/credential/readiness state when ready", async () => {
    ...
    expect(
      renderer.root.findByProps({ "data-readiness": "bound" }),
    ).toBeTruthy();
    renderer.unmount();
  });
  ...
  it("blocks saving when a required value is cleared", async () => {
    ...
    await act(async () => {
      field(renderer, "homeserverUrl").props.onChange({
        target: { value: "" },
      });
    });
  ...
  it("does not write settings or credentials from a read-only scope", async () => {
    ...
    await act(async () => {
      field(renderer, "roomId").props.onChange({
        target: { value: "!draft:example" },
      });
      field(renderer, "accessToken").props.onChange({
  ...
  it("replaces a configured credential and clears only after acceptance", async () => {
    ...
    expect(credentials.setCalls).toEqual([
      { ref: "DSH_MATRIX_ACCESS_TOKEN", value: "replacement-token" },
    ]);
    await act(async () => {
      renderer.root
        .findByProps({ "data-plugin-card-header": "dsh-matrix" })
        .props.onClick();
    });
    expect(field(renderer, "accessToken").props.value).toBe("");
    renderer.unmount();
    ...
    expect(
      renderer.root
        ...
        .some(
          (node) =>
            node.props.children ===
            "The deployment rejected these values; your draft was kept.",
        ),
    ).toBe(true);
    renderer.unmount();
  });
  ...
  it.each([
    ...
    "unbound",
    "failed",
  ] as const)("renders readiness state %s", async (state) => {
    const fixture = scopeFixture();
    const renderer = await renderCard(
      props(fixture, apiFixture(), {
        get: async () => ({ ok: true, value: { state } }),
      }),
    );
  ...
  it("keeps a dirty draft across an external refresh and supports discard/save", async () => {
    ...
    fixture.publish({
      ...
      value: {
        ...fixture.scope.getSnapshot().value,
        roomId: "!external:example",
      },
    });
    expect(field(renderer, "roomId").props.value).toBe("!draft:example");
    ...
    await act(async () => {
      field(renderer, "respondToAll").props.onChange({
        target: { checked: true },
      });
    });
    await act(async () => {
