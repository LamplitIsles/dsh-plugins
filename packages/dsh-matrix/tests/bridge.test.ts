...
function baseDeps(client: FakeClient, agent?: BridgeAgent): BridgeDependencies {
  return {
    ...
    workspaceRegistry: {
      get: () => ({ id: "workspace", sessionIds: ["session"] }),
      archivedSessionIds: new Set(),
    },
    inspectSession: async () => ({
      meta: { id: "session", agentPreset: "default" },
      events: [
        { type: "user/message", time: 1, data: { source: { kind: "user" } } },
      ],
    }),
    resolveAgent: async () =>
      agent ? { agent } : { error: new Error("agent unavailable") },
...
describe("MatrixBridge", () => {
  it("resolves the selected conversation once through the shared controller", async () => {
    ...
    dependencies.workspaceRegistry = {
      get: () => ({ id: "workspace", sessionIds: ["older", "newer"] }),
      archivedSessionIds: new Set(),
    };
    dependencies.inspectSession = async (sessionId) => ({
      meta: { id: sessionId },
      events: [
        {
          type: "user/message",
          time: sessionId === "newer" ? 2 : 1,
          data: { source: { kind: "user" } },
        },
      ],
    });
    const resolvedSessionIds: string[] = [];
    dependencies.resolveAgent = async (sessionId) => {
  ...
  it("does not start Matrix when shared Agent resolution fails", async () => {
    ...
    let matrixStarts = 0;
    dependencies.matrixClientFactory = async () => {
      matrixStarts += 1;
      return client;
    };
    const bridge = new MatrixBridge(dependencies);
  ...
  it("registers only the active Companion's four fixed-target tools and scoped policy", async () => {
      ...
      ctx: {
        ...
        systemPrompt: {
          section: (section: { name: string }) => {
            disposed.push(`policy:${section.name}`);
            return () => undefined;
          },
        },
      } as never,
    ...
    await registered[2]!.execute({ body: "hello" }, {
  ...
  it("does not relay a final Assistant text without an explicit send call", async () => {
    ...
    const agent: BridgeAgent = {
      id: "session" as never,
      followup: (message) => {
        prompts.push(message as UserMessage);
      },
      whenIdle: async () => undefined,
    };
    const bridge = new MatrixBridge(baseDeps(client, agent));
    ...
    client.emit(
      "Room.timeline",
      matrixEvent("$trigger", "@bot:example answer"),
      {},
      false,
      false,
      { timeline: "live" },
  ...
  it("allows the Agent to reply to verified room history from any turn", async () => {
    const client = new FakeClient();
    client.fetchRoomEvent = async (_roomId, eventId) =>
      matrixEvent(eventId, "earlier");
    const registered: ToolDefinition[] = [];
    const agent: BridgeAgent = {
      id: "session" as never,
      ctx: {
      ...
      followup: (message) => {
        void message;
        const send = registered.find(
          (tool) => tool.name === MATRIX_SEND_MESSAGE,
        )!;
        return send.execute({ body: "reply", replyToEventId: "$older" }, {
          signal: new AbortController().signal,
        } as never) as never;
      },
      whenIdle: async () => undefined,
  ...
  it("serves bounded readiness through the package RPC", async () => {
    ...
    await expect(
      handler("unknown", {}, new AbortController().signal),
