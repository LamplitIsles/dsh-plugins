...
class FakeFs implements MatrixFileSystemLike {
  ...
  async stat(
    ...
    return current
      ? {
          type: current.kind,
          ...(current.data ? { size: current.data.byteLength } : {}),
        }
      : undefined;
  }
...
function mediaSetup(
  ...
  const agent = {
    ...
    ctx: {
      get: (name: string) =>
        name === "fs" ? fs : name === "dshSpeech" ? options.speech : undefined,
    },
  };
  ...
  return {
    ...
    client,
    agent,
    send: definitions.find((tool) => tool.name === MATRIX_SEND_MESSAGE)!,
    file: definitions.find((tool) => tool.name === MATRIX_SEND_FILE)!,
  };
}
...
describe("Matrix media delivery", () => {
      ...
      send.execute(
        {
          body: "voice body",
          voice: true,
          replyToEventId: "$history",
          mentions: ["Alice"],
        },
        exec(controller.signal, setup.agent),
      ),
  ...
  it("fails voice calls explicitly when DSH Speech is absent or returns invalid audio", async () => {
    ...
    await expect(
      absent.send.execute({ body: "voice", voice: true }, exec()),
    ).rejects.toThrow(/voice delivery is unavailable/);
    expect(absent.client.sent).toHaveLength(0);
    const invalid = mediaSetup({
      speech: {
        synthesize: async () => ({
          mediaType: "audio/wav",
    ...
    await expect(
      invalid.send.execute({ body: "voice", voice: true }, exec()),
  ...
  it("sends image media with a description and file media with basename fallback", async () => {
    const fs = new FakeFs();
    fs.targets.set(`${WORKSPACE}/plot.PNG`, {
      displayPath: `${WORKSPACE}/plot.PNG`,
      kind: "file",
      data: new Uint8Array([9, 8]),
    });
    fs.targets.set(`${WORKSPACE}/report.txt`, {
      displayPath: `${WORKSPACE}/report.txt`,
      kind: "file",
      data: new Uint8Array([7, 6, 5]),
    });
    const setup = mediaSetup({ fs });
    await setup.file.execute(
      { path: "plot.PNG", description: "A plot", replyToEventId: "$history" },
      exec(),
    );
  ...
  it("rejects outside, missing, non-regular, oversize, unreadable, and malformed paths", async () => {
    const fs = new FakeFs();
    fs.targets.set(`${WORKSPACE}/dir`, {
      displayPath: `${WORKSPACE}/dir`,
      kind: "directory",
    });
    fs.targets.set(`${WORKSPACE}/large.bin`, {
      displayPath: `${WORKSPACE}/large.bin`,
      kind: "file",
      data: new Uint8Array(MAX_MATRIX_MEDIA_BYTES + 1),
    });
    fs.targets.set(`${WORKSPACE}/secret.bin`, {
      displayPath: `${WORKSPACE}/secret.bin`,
      kind: "file",
      data: new Uint8Array([1]),
    });
    const setup = mediaSetup({ fs });
    ...
    for (const [path, expected] of [
      ...
      await expect(setup.file.execute({ path }, exec())).rejects.toThrow(
        expected,
      );
    }
  ...
  it("maps cancellation, readiness loss, upload failure, and send failure without text fallback", async () => {
    ...
    controller.abort();
    await expect(
      canceled.file.execute({ path: "report.txt" }, exec(controller.signal)),
    ...
    await expect(
      readiness.file.execute({ path: "report.txt" }, exec()),
