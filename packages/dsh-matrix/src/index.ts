...
export function apply(ctx: HostContext): void {
  ...
  const bridge = new MatrixBridge({
    ...
    inspectSession: async (sessionId) =>
      (await ctx.sessionController.inspect(sessionId)) as any,
    resolveAgent: async (sessionId) =>
      await ctx.sessionController.resolveAgent(sessionId),
    ...
    onError: (error) => {
      ...
      if (process.env.NODE_ENV !== "test")
        console.error("[dsh-matrix] bridge error");
    },
  });
...
export { MatrixBridge, bridgeRpcHandler } from "./bridge.js";
export type {
  BridgeAgent,
  BridgeDependencies,
  BridgeReadiness,
