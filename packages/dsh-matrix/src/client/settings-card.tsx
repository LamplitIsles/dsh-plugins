...
const EMPTY_CREDENTIAL: CredentialStatus = {
  configured: false,
  writable: false,
};
...
function readinessValue(value: unknown): BridgeReadiness {
  ...
  const allowed: readonly BridgeReadinessState[] = [
    "disabled",
    "missing-settings",
    "missing-credential",
    "connecting",
  ...
  return {
    state: allowed.includes(state as BridgeReadinessState)
      ? (state as BridgeReadinessState)
      : "failed",
  };
}

function snapshotValue(
  snapshot: SettingsScopeSnapshot<Partial<MatrixSettings>>,
): MatrixSettings {
  return normalizeSettings(snapshot.value);
}

function fieldValue(
  baseline: MatrixSettings,
  draft: Partial<MatrixSettings>,
  field: keyof MatrixSettings,
): string | boolean {
...
function PluginCardFrame(props: CardFrameProps) {
  ...
  useEffect(() => {
    ...
    if (!props.state.dirty && !props.state.failed) {
      // oxlint-disable-next-line react/set-state-in-effect -- close after parent save completion.
      setOpen(false);
    }
  ...
  return createElement(
    "li",
    {
      ...
      "data-plugin-card": SETTINGS_NAMESPACE,
    },
    createElement(
      "button",
      {
    ...
    open
      ? createElement(
          "div",
          { id: bodyId, className: styles.body },
          !props.state.writable
            ? createElement(
                "p",
                { className: styles.status, role: "status" },
                props.readOnlyLabel,
              )
            : null,
          props.children,
          createElement(
            ...
            props.state.failed
              ? createElement(
                  "p",
                  {
                    className: `${styles.status} ${styles.invalid}`,
                    role: "status",
                  },
                  props.failedLabel,
                )
            ...
            createElement(
              ...
              props.state.saving ? props.savingLabel : props.saveLabel,
            ),
          ),
        )
      : null,
...
export function MatrixSettingsCard({
  ...
  const [credentials, setCredentials] =
    useState<CredentialStatus>(EMPTY_CREDENTIAL);
  ...
  useEffect(() => {
    ...
    void describeCredential(api).then((status) => {
      if (active) setCredentials(status);
    });
    return () => {
      active = false;
    };
  ...
  const workspaceStale = Boolean(
    effective.workspaceId &&
    workspaceState.phase === "ready" &&
    !choices.some((choice) => choice.id === effective.workspaceId),
  );
  const invalid = !validation.valid || workspaceStale;

  const edit = <K extends keyof MatrixSettings>(
    field: K,
  ...
  const input = (
    ...
    return createElement(
      ...
      createElement(
        "label",
        { className: styles.label, htmlFor: `${cardId}-${field}` },
        text(labelKey),
      ),
      createElement("input", {
        id: `${cardId}-${field}`,
        ...
        onChange: (event: { target: { value: string } }) =>
          edit(field, event.target.value as MatrixSettings[typeof field]),
      ...
      createElement(
        "p",
        {
          className: `${styles.hint} ${issue ? styles.invalid : ""}`,
          id: `${cardId}-${field}-hint`,
        },
        issue === "required"
          ? text("required")
          : issue === "url"
  ...
  const runtimeText =
    runtime.state === "unbound" ? text("unbound") : text(runtime.state);
  return createElement(
    PluginCardFrame,
    {
      title: text("title"),
    ...
    createElement(
      "div",
      { className: styles.form, "data-settings-card": SETTINGS_NAMESPACE },
      createElement(
        "div",
        ...
        createElement(
          "div",
          { className: styles.field, key: "workspaceId" },
          createElement(
            "label",
            { className: styles.label, htmlFor: `${cardId}-workspaceId` },
            text("workspaceId"),
          ),
          createElement(
            "select",
            {
              ...
              disabled: !writable || saving,
              onChange: (event: { target: { value: string } }) =>
                edit("workspaceId", event.target.value),
          ...
          createElement(
            "p",
            {
              className: `${styles.hint} ${workspaceStale ? styles.invalid : ""}`,
              id: `${cardId}-workspaceId-hint`,
            },
            workspaceStale ? text("workspaceMissing") : text("workspaceHint"),
          ),
        ),
        createElement(
          "div",
          { className: styles.field, key: "accessToken" },
          createElement(
            "label",
            { className: styles.label, htmlFor: `${cardId}-accessToken` },
            text("accessToken"),
          ),
          createElement("input", {
          ...
          createElement(
            "span",
            {
              className: credentials.configured
                ? styles.credentialState
                : styles.credentialUnset,
              "data-credential-configured": credentials.configured
                ? "yes"
                : "no",
            },
            credentials.configured ? text("configured") : text("notConfigured"),
          ),
          createElement(
            "p",
            { className: styles.hint, id: `${cardId}-accessToken-hint` },
            text("accessTokenHint"),
          ),
        ),
      ),
      createElement(
        "label",
        { className: styles.toggle, htmlFor: `${cardId}-respondToAll` },
        createElement("input", {
          ...
          onChange: (event: { target: { checked: boolean } }) =>
            edit("respondToAll", event.target.checked),
