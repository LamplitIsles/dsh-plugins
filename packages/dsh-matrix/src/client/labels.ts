...
export const matrixLabels: Record<MatrixLocaleKey, string> = {
  ...
  homeserverHint:
    "The homeserver/API URL used by Element, for example https://matrix.example.com. It may differ from the domain in the Matrix user ID.",
  userId: "Matrix user ID",
  userIdHint:
    "Enter the complete ID, for example @bot:matrix.example.com, not only bot.",
  roomId: "Allowed room ID",
  roomIdHint:
    "Enter the complete room ID, for example !abcdef:matrix.example.com, not a #room:example.com alias.",
  workspaceId: "Companion workspace",
  workspaceHint:
    "The workspace whose active conversation is locked at startup.",
  workspaceMissing:
    "The selected workspace is not available in this DSH deployment.",
  accessToken: "Element access token",
  accessTokenHint:
    "Use an Element access token belonging to the Matrix user above. It is write-only; leave blank to keep the current token.",
  configured: "Configured",
  notConfigured: "Not configured",
  respondToAll: "Respond to all messages",
  respondToAllHint:
    "Off means the bot requires a mention or a reply to its message.",
  runtime: "Runtime readiness",
  restartHint:
    "Changes apply after restarting DSH; the bound conversation never switches live.",
  unbound:
    "Connected, but no eligible existing Companion conversation was found.",
  save: "Save",
  saving: "Saving…",
  discard: "Discard",
  unsaved: "Unsaved",
...
export const matrixZhLabels: Record<MatrixLocaleKey, string> = {
  ...
  homeserverUrl: "Homeserver URL",
  homeserverHint:
    "填写 Element 实际连接的 homeserver/API 地址，例如 https://matrix.example.com；它可以与 Matrix 用户 ID 的域名不同。",
  userId: "Matrix 用户 ID",
  userIdHint: "填写完整 ID，例如 @bot:matrix.example.com，而不是只填 bot。",
  roomId: "允许的房间 ID",
  roomIdHint:
    "填写完整 room ID，例如 !abcdef:matrix.example.com；不要填写 #room:example.com 房间别名。",
  workspaceId: "Companion Workspace",
  workspaceHint: "插件启动时会固定绑定这个 Workspace 中最近活跃的现有会话。",
  workspaceMissing: "当前 DSH 中找不到之前选择的 Workspace，请重新选择。",
  accessToken: "Element access token",
  accessTokenHint:
    "填写属于上述 Matrix 用户的 Element access token。该字段只写；留空会保留现有 token。",
  configured: "已配置",
  notConfigured: "未配置",
  respondToAll: "响应所有消息",
  ...
  unbound:
    "Matrix 已连接，但所选 Workspace 中没有可绑定的现有 Companion 会话。",
  save: "保存",
  saving: "正在保存…",
  discard: "放弃修改",
  unsaved: "未保存",
