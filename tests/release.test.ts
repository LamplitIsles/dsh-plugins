...
describe("automatic releases", () => {
  ...
  function commit(root: string) {
    git(root, "add", ".");
    git(root, "-c", "commit.gpgsign=false", "commit", "--quiet", "--allow-empty", "-m", "fixture");
    return git(root, "rev-parse", "HEAD");
  }
  async function manifest(root: string, directory: string, version: string, extra = {}) {
    ...
    await writeFile(join(root, "packages", directory, "package.json"), JSON.stringify({
      name: `@lamplitisles/${directory}`, version, ...extra,
    }));
  }
  ...
  it("does not turn new manifests or initial pushes into bulk releases", async () => {
    ...
    const after = commit(root);
    expect(releaseChanges(root, before, after)).toEqual([]);
    expect(() => releaseChanges(root, "0".repeat(40), after)).toThrow("existing commit SHAs");
    expect(() => releaseChanges(root, "main", after)).toThrow("existing commit SHAs");
    expect(() => releaseChanges(root, after, before)).toThrow();
  });
  ...
  it("rejects invalid versions and changed package identities", async () => {
    ...
    expect(() => releaseChanges(root, before, commit(root))).toThrow("v<semver>");
    await manifest(root, "dsh-companion", "0.3.0", { name: "unexpected" });
    expect(() => releaseChanges(root, before, commit(root))).toThrow("Unexpected package identity");
  });
